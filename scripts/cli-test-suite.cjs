...
// Try to use chalk, fallback to basic colors if not available
let chalk;
try {
  chalk = require('chalk');
} catch (e) {
  // Fallback chalk if module not available
  chalk = {
...
class CLITestSuite {
  ...
  generateReport() {
    ...
    const templateErrors = this.results.failing.filter(r => r.error && r.error.includes('unexpected token: %'));
    if (templateErrors.length > 0) {
      this.log('  • Template parsing errors detected (Nunjucks syntax issues)', 'yellow');
    }

    const cittyErrors = this.results.failing.filter(r => r.error && r.error.includes('Cannot find package \'citty\''));
    if (cittyErrors.length > 0) {
      this.log('  • Citty import failures (dependency resolution issues)', 'red');
    }

    const workingShellCommands = this.results.working.filter(r => r.output && r.output.includes('v2025.9.08.1'));
