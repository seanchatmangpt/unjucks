...
/**
 * @typedef {Object} TypeScriptGenerationOptions
 * @property {string} namespace - TypeScript namespace
 * @property {string} outputFormat - Output format (interface|type|class)
 * @property {boolean} includeValidation - Include validation decorators
 * @property {boolean} includeDocumentation - Include JSDoc documentation
 * @property {boolean} enterpriseTypes - Include enterprise-specific type mappings
 */

/**
 * @typedef {Object} OntologyGenerationOptions
 * @property {string} baseUri - Base URI for generated ontology
 * @property {string} format - Output format (turtle|rdf-xml|n3|json-ld)
 * @property {boolean} useOWL - Use OWL constructs vs pure RDFS
 * @property {boolean} generateShapes - Generate SHACL validation shapes
 * @property {string} namespacePrefix - Namespace prefix for classes
 */
...
export const generateInterfaces = defineCommand({
   ...
   * @param {any} context.args - Command arguments
   */
  async run({ args }) {
    try {
...
export const generateOntology = defineCommand({
  ...
  async run({ args }) {
    try {
      ...
      console.error(`✗ Error generating ontology: ${error.message}`);
      process.exit(1);
    }
  }
});
...
export const validateRoundTrip = defineCommand({
  ...
  async run({ args }) {
    try {
      ...
      console.error(`✗ Round-trip validation failed: ${error.message}`);
      process.exit(1);
    }
  }
});
