...
import type { ToolResult, UnjucksHelpParams } from '../types.js';
import { 
 ...
 */
export async function unjucksHelp(params: UnjucksHelpParams): Promise<ToolResult> {
  ...
  try {
    ...
    try {
      ...
      const templateConfig = generatorConfig.templates?.find((t: any) => t.name === template) || {};
 ...
 */
async function getTemplateFilesWithContent(templatePath: string) {
  const files = [];
      ...
      if (stat.isDirectory()) {
        ...
        try {
          ...
          files.push({
            ...
            frontmatter,
            variables,
            description: (frontmatter as any).description || generateFileDescription(entry, variables),
 ...
 */
function generateUsageExamples(generator: string, template: string, variables: any[]) {
  const examples = [];
 ...
 */
function getExampleValue(variable: any): string {
 ...
 */
function generateVariableExamples(name: string, type: string): string[] {
  const examples = [];
 ...
 */
function generateFileDescription(filename: string, variables: string[]): string {
 ...
 */
function generateHelpTips(generator: string, template: string, variables: any[]): string[] {
  const tips = [];
 ...
 */
function formatHelpAsText(helpInfo: any): string {
  const lines = [];
