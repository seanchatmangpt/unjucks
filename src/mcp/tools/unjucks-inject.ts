...
import type { ToolResult, UnjucksInjectParams } from '../types.js';
import { 
 ...
 */
export async function unjucksInject(params: UnjucksInjectParams): Promise<ToolResult> {
 ...
 */
function getInjectionMethod(config: any): string {
 ...
 */
function getInjectionTarget(config: any): string {
 ...
 */
function simulateInjection(existingContent: string, newContent: string, config: any): string {
 ...
 */
function formatInjectionReport(report: any): string {
  const lines = [];
