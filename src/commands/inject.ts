...
import type {
  InjectCommandArgs,
  CommandResult,
  InjectionMode as CommandInjectionMode,
...
export const injectCommand = defineCommand({
  ...
  args: {
    file: {
      type: "positional",
      description:
        "Target file to modify (must exist unless creating new file)",
      required: true,
    },
    content: {
      type: "string",
    ...
    template: {
      type: "string",
      description:
        "Template name to render and inject (alternative to --content)",
      alias: "t",
    },
    generator: {
      type: "string",
      description:
        "Generator containing the template (required with --template)",
      alias: "g",
    },
    mode: {
      type: "string",
      description:
        "Injection mode: inject, append, prepend, before, after, replace",
      default: "inject",
      alias: "m",
    },
    target: {
      type: "string",
      description:
        "Target marker/pattern for injection (used with inject/before/after modes)",
      alias: "T",
    },
    line: {
      type: "string",
    ...
    skipIf: {
      type: "string",
      description:
        "Skip injection if condition is met (e.g., content already exists)",
      alias: "s",
    },
    verbose: {
      type: "boolean",
  ...
  async run(context: any) {
    ...
    try {
      ...
      const validationResults = [
        ...
        validators.required(
          { content: args.content, template: args.template },
          ["content or template"]
        ),
      ];
      ...
      if (args.content) {
        ...
        // Render template with variables - use generate for now
        const templateResult = await generator.generate({
          generator: args.generator,
          template: args.template,
          dest: '/tmp', // Temporary destination for rendering
          variables: args.vars || {},
          force: false,
          dry: true // Don't actually write files
        });

        // Extract content from the first generated file
        contentToInject = templateResult.files.length > 0 ? templateResult.files[0].content || '' : '';

        if (args.verbose) {
          console.log(chalk.gray("Rendered content preview:"));
          console.log(
            chalk.gray(
              contentToInject.substring(0, 200) +
                (contentToInject.length > 200 ? "..." : "")
            )
          );
        }
      } else {
      ...
      if (!args.quiet) {
        const modeDesc = getModeDescription(
          args.mode as InjectionMode,
          args.target,
          args.line
        );
        console.log(
          chalk.green(
            `\\n🎯 ${modeDesc} in: ${chalk.bold(
      ...
      if (result.success) {
        ...
        throw createCommandError(result.message, CommandError.UNKNOWN_ERROR, [
      ...
      if (error instanceof UnjucksCommandError) {
        ...
        console.log(chalk.blue("\\n💡 Suggestions:"));
        console.log(
          chalk.blue("  • Check that the target file exists and is writable")
        );
        console.log(
          chalk.blue("  • Verify template exists if using template mode")
        );
        console.log(
...
function getModeDescription(
  ...
  switch (mode) {
    ...
    case "before":
      return target
        ? `Inserting content before "${target}"`
        : "Inserting content (before mode)";
    case "after":
      return target
        ? `Inserting content after "${target}"`
        : "Inserting content (after mode)";
    case "replace":
      return target
        ? `Replacing content matching "${target}"`
        : "Replacing content";
    case "inject":
      if (line) {
