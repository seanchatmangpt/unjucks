...
export interface PersonalDataElement {
  id: string;
  name: string;
  category: 'identifying' | 'sensitive' | 'financial' | 'health' | 'biometric' | 'location' | 'behavioral';
  description: string;
...
export interface DataSubjectRequest {
  ...
  responseMethod: 'email' | 'portal' | 'postal' | 'in_person';
  verificationMethod: string;
}

export interface GDPRViolation {
  id: string;
  principle: 'lawfulness' | 'fairness' | 'transparency' | 'purpose_limitation' | 'data_minimization' | 'accuracy' | 'storage_limitation' | 'security' | 'accountability';
  article: string;
  ...
  remediation: string[];
  status: 'open' | 'in_progress' | 'resolved' | 'reported_to_authority';
...
export interface GDPRCheckerConfig {
  ...
  consentRecordsDir: string;
  privacyNoticesDir: string;
  generateDataMap: boolean;
  performPIA: boolean;
...
export class GDPRComplianceChecker {
  private config: GDPRCheckerConfig;
  private dataElements: PersonalDataElement[] = [];
  ...
  private ensureDirectories(): void {
    [
      ...
      join(this.config.outputDir, 'data-maps'),
      join(this.config.outputDir, 'pia-reports'),
      join(this.config.outputDir, 'breach-reports')
    ].forEach(dir => {
      try {
  ...
  private loadDataInventory(): void {
    try {
      if (existsSync(this.config.dataInventoryFile)) {
        const content = readFileSync(this.config.dataInventoryFile, 'utf-8');
  ...
  private getDefaultDataElements(): PersonalDataElement[] {
    return [
      {
        id: 'email_address',
        name: 'Email Address',
  ...
  private getDefaultProcessingActivities(): DataProcessingActivity[] {
    return [
      {
  ...
  private saveDataInventory(): void {
    ...
    writeFileSync(this.config.dataInventoryFile, content, 'utf-8');
  }

  private loadConsentRecords(): void {
    try {
      const consentFiles = readdirSync(this.config.consentRecordsDir)
        .filter(file => file.endsWith('.json'));

      this.consentRecords = [];
      for (const file of consentFiles) {
        const filePath = join(this.config.consentRecordsDir, file);
  ...
  private loadDataSubjectRequests(): void {
    try {
      const requestFile = join(this.config.outputDir, 'data-subject-requests.json');
  ...
  private checkDataProcessingCompliance(): void {
  ...
  private checkRetentionCompliance(): void {
  ...
  private checkPrivacyNotices(): { upToDate: boolean; lastReviewed: string; languages: string[] } {
    console.log('🔍 Checking privacy notices...');

    try {
      const noticeFiles = readdirSync(this.config.privacyNoticesDir)
      ...
      for (const file of noticeFiles) {
        const filePath = join(this.config.privacyNoticesDir, file);
  ...
  private generateDataSubjectRightsStats(): any {
    ...
    const recentRequests = this.dataSubjectRequests.filter(req => 
      new Date(req.requestDate) > thirtyDaysAgo
    );

    const completedOnTime = recentRequests.filter(req => 
      req.status === 'completed' && 
      req.completedDate &&
      new Date(req.completedDate) <= new Date(req.dueDate)
    );

    const overdue = recentRequests.filter(req => 
      req.status !== 'completed' && 
  ...
  private generatePIAReports(): void {
    console.log('📋 Generating Privacy Impact Assessment reports...');

    const highRiskActivities = this.processingActivities.filter(a => 
  ...
  private generateDataMaps(): void {
    ...
    writeFileSync(dataMapPath, content, 'utf-8');
  ...
  private saveGDPRReport(report: GDPRComplianceReport): void {
    const reportPath = join(this.config.outputDir, `gdpr-compliance-report-${report.reportId}.json`);
  ...
  private generateGDPRExecutiveSummary(report: GDPRComplianceReport): void {
    const summaryPath = join(this.config.outputDir, `gdpr-executive-summary-${report.reportId}.md`);
  ...
  private generateGDPRDetailedReport(report: GDPRComplianceReport): void {
    const detailPath = join(this.config.outputDir, `gdpr-detailed-report-${report.reportId}.md`);
...
if (require.main === module) {
  const config: GDPRCheckerConfig = {
