/**
 * Final CLI Commands - BDD Compliant Implementation
 * 
...
import EnhancedValidationEngine from './cli-validation-enhanced.js';
import { getDeterministicISOString } from './utils/deterministic-time.js';

// Initialize enhanced CLI engines
const enhancedCLI = new EnhancedCLIEngine();
const enhancedValidator = new EnhancedValidationEngine();
...
export const enhancedValidateGraph = defineCommand({
  ...
  async run({ args }) {
    try {
      await enhancedValidator.initialize({ debug: args.debug });
      ...
      const result = await enhancedValidator.validateGraph(args.file, options);
...
export { enhancedCLI, enhancedValidator };