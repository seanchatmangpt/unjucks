...
import type {
  ParsedArguments as UnifiedParsedArguments,
  ParseContext as UnifiedParseContext,
...
export class PositionalParser {
  ...
  async parseHygenStyle(
    ...
    if (args.length < 2) {
      throw new Error(
        "Hygen-style parsing requires at least generator and template"
      );
    }
    ...
    const { flags, additionalPositionals } =
      this.parseRemainingArgs(remainingArgs);
  ...
  async parseUnjucksStyle(
    ...
    if (actualArgs.length < 2) {
      throw new Error(
        "Unjucks-style parsing requires at least generator and template"
      );
    }
      ...
      if (mapping && !result[mapping.variableName]) {
        ...
        try {
          ...
          if (mapping.validator && !mapping.validator(arg)) {
            throw new Error(
              `Invalid value for ${mapping.variableName}: ${arg}`
            );
          }
   ...
   */
  private parseRemainingArgs(args: string[]): {
    flags: Record<string, any>;
  ...
  private parseFlag(
    args: string[],
    index: number
  ): { key: string; value: any; nextIndex: number } {
   ...
   */
  private async loadTemplateVariables(
    generator: string,
    template: string
  ): Promise<TemplateVariable[]> {
    try {
      const { variables } = await this.generator.scanTemplateForVariables(
        generator,
        template
      );
      return variables;
      ...
      if (this.config.debugMode) {
        console.warn(
          `[PositionalParser] Could not load variables for ${generator}/${template}:`,
          error
        );
      }
      return [];
   ...
   * Sort variables by priority for positional mapping
   */
  private sortVariablesByPriority(
    variables: TemplateVariable[]
  ): TemplateVariable[] {
