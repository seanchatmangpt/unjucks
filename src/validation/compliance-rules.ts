...
export class GDPRValidator {
  private readonly personalDataIndicators = [
    // Direct identifiers
  ...
  private readonly requiredGDPRProperties = [
  ...
  validate(data: TurtleData): ValidationError[] {
  ...
  private validatePersonalDataProtection(data: TurtleData, violations: ValidationError[]): void {
  ...
  private validateConsentTracking(data: TurtleData, violations: ValidationError[]): void {
    for (const [subjectUri, resource] of Object.entries(data.subjects)) {
  ...
  private validatePurposeLimitation(data: TurtleData, violations: ValidationError[]): void {
    for (const [subjectUri, resource] of Object.entries(data.subjects)) {
      if (this.containsPersonalData(resource)) {
  ...
  private validateDataRetention(data: TurtleData, violations: ValidationError[]): void {
  ...
  private validateDataSubjectRights(data: TurtleData, violations: ValidationError[]): void {
    for (const [subjectUri, resource] of Object.entries(data.subjects)) {
      if (this.containsPersonalData(resource)) {
  ...
  private containsPersonalData(resource: RDFResource): boolean {
  ...
  private hasDataProtectionMeasures(resource: RDFResource): boolean {
  ...
  private hasConsentTracking(resource: RDFResource): boolean {
...
export class HIPAAValidator {
  private readonly phiIndicators = [
    // Medical identifiers
  ...
  private readonly requiredSafeguards = [
  ...
  private validatePHIProtection(data: TurtleData, violations: ValidationError[]): void {
    for (const [subjectUri, resource] of Object.entries(data.subjects)) {
  ...
  private validateAdministrativeSafeguards(data: TurtleData, violations: ValidationError[]): void {
  ...
  private validatePhysicalSafeguards(data: TurtleData, violations: ValidationError[]): void {
  ...
  private validateTechnicalSafeguards(data: TurtleData, violations: ValidationError[]): void {
    for (const [subjectUri, resource] of Object.entries(data.subjects)) {
      if (this.containsPHI(resource)) {
  ...
  private validateMinimumNecessary(data: TurtleData, violations: ValidationError[]): void {
    for (const [subjectUri, resource] of Object.entries(data.subjects)) {
      if (this.containsPHI(resource)) {
  ...
  private containsPHI(resource: RDFResource): boolean {
    const allValues = Object.values(resource.properties)
      .flat()
      .map(v => v.value.toLowerCase())
      .join(' ');
  ...
  private hasEncryption(resource: RDFResource): boolean {
  ...
  private hasAccessControls(resource: RDFResource): boolean {
...
export class SOXValidator {
  private readonly financialIndicators = [
    // Financial data
  ...
  private validateFinancialDataControls(data: TurtleData, violations: ValidationError[]): void {
    for (const [subjectUri, resource] of Object.entries(data.subjects)) {
  ...
  private validateAuditTrails(data: TurtleData, violations: ValidationError[]): void {
    for (const [subjectUri, resource] of Object.entries(data.subjects)) {
      if (this.containsFinancialData(resource)) {
  ...
  private validateInternalControls(data: TurtleData, violations: ValidationError[]): void {
  ...
  private validateChangeManagement(data: TurtleData, violations: ValidationError[]): void {
    for (const [subjectUri, resource] of Object.entries(data.subjects)) {
      if (this.containsFinancialData(resource)) {
        const changeProps = [
          'http://purl.org/dc/terms/modified',
  ...
  private containsFinancialData(resource: RDFResource): boolean {
    const allValues = Object.values(resource.properties)
      .flat()
      .map(v => v.value.toLowerCase())
      .join(' ');
  ...
  private hasInternalControls(resource: RDFResource): boolean {
    const controlProps = [
  ...
  private hasApprovalProcess(resource: RDFResource): boolean {
    const approvalProps = [
...
export type ComplianceFramework = keyof typeof ComplianceRules;