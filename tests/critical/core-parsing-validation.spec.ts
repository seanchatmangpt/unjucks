...
import type { TurtleParseResult } from '../../src/lib/turtle-parser.js';
...
describe('80/20 Core Parsing Validation', () => {
  ...
  test('Parse enterprise-scale ontology with business entities', async () => {
    // Load real enterprise-style data
    ...
    const result: TurtleParseResult = await parser.parse(enterpriseData);
    ...
    // 4. Business entity validation - core enterprise types
    const businessEntities = result.triples.filter(triple => 
      triple.predicate.value.includes('type') && (
        triple.object.value.includes('Organization') ||
        triple.object.value.includes('Person') ||
        triple.object.value.includes('SoftwareApplication')
      )
    );
    ...
    expect(result.namedGraphs).toBeInstanceOf(Array);
    
    // 6. Triple structure validation
    const sampleTriple = result.triples[0];
    expect(sampleTriple).toHaveProperty('subject');
    expect(sampleTriple).toHaveProperty('predicate');  
    expect(sampleTriple).toHaveProperty('object');
    expect(sampleTriple.subject).toHaveProperty('type');
    expect(sampleTriple.subject).toHaveProperty('value');
  });
  ...
  test('Parse multi-vocabulary RDF with cross-references', async () => {
    const multiVocabData = `
      ...
      <#enterprise> a org:Organization ;
          ...
          org:hasMember <#ceo> ;
          schema:numberOfEmployees "100000"^^xsd:integer .
          
      <#ceo> a foaf:Person ;
          schema:jobTitle "Chief Executive Officer" ;
          foaf:name "Executive Leader" ;
          org:memberOf <#enterprise> ;
          schema:worksFor <#enterprise> .
          
      <#paymentService> a schema:SoftwareApplication ;
          schema:name "Payment Processing Service" ;
          schema:applicationCategory "FinancialSoftware" ;
          org:organizationOf <#enterprise> .
    `;
    ...
    // 1. All vocabularies recognized
    expect(result.prefixes).toHaveProperty('foaf');
    expect(result.prefixes).toHaveProperty('org');
    expect(result.prefixes).toHaveProperty('schema');
    expect(result.prefixes).toHaveProperty('xsd');
    
    // 2. Entity relationships preserved
    const enterpriseEntity = result.triples.find(triple => 
      triple.subject.value === '#enterprise' && 
      triple.predicate.value.includes('type') &&
      triple.object.value.includes('Organization')
    );
    ...
    const ceoRelationship = result.triples.find(triple =>
      triple.subject.value === '#enterprise' &&
      triple.predicate.value.includes('hasMember') &&
      triple.object.value === '#ceo'
    );
    expect(ceoRelationship).toBeDefined();
    
    // 4. Datatype handling
    const typedLiterals = result.triples.filter(triple =>
      triple.object.datatype && triple.object.datatype.includes('xsd:')
    );
    expect(typedLiterals.length).toBeGreaterThan(0);
    
    // 5. Minimum enterprise entities
    expect(result.triples.length).toBeGreaterThan(10);
    expect(result.stats.tripleCount).toBeGreaterThan(10);
  });
  ...
  test('Extract template variables from semantic data', async () => {
    ...
    const allPersonProperties = personTriples.map(triple => ({
      property: triple.predicate.value.split('/').pop()?.split('#').pop(),
  ...
  test('Handle parsing errors with detailed diagnostics', async () => {
    const invalidData = `
      @prefix foaf: <http://xmlns.com/foaf/0.1/> .
