...
describe('Positional Parameters - Integration Tests', () => {
  ...
  it('should show enhanced usage information with positional syntax examples', () => {
    const result = testWithExistingTemplates();
    
    expect(result).toContain('🌆 Unjucks CLI');
    expect(result).toContain('Usage:');
    expect(result).toContain('unjucks <generator> <template> [args...]  # Positional syntax (Hygen-style)');
    expect(result).toContain('unjucks generate <generator> <template>   # Explicit syntax');
    expect(result).toContain('Examples:');
    expect(result).toContain('unjucks component new MyComponent     # Positional style');
    expect(result).toContain('unjucks generate component citty      # Explicit style');
  });
  ...
  it('should list generators correctly', () => {
    ...
    // Should contain existing test generators
  ...
  describe('Hygen Parity Achievement', () => {
    it('should validate that the critical gap from HYGEN-DELTA.md is implemented', () => {
      // From HYGEN-DELTA.md: 
      ...
      // Test 1: The preprocessing functionality is in place
      const cliContent = fs.readFileSync('/Users/sac/unjucks/src/cli.ts', 'utf-8');
      expect(cliContent).toContain('preprocessArgs');
      expect(cliContent).toContain('process.argv = [...process.argv.slice(0, 2), ...preprocessArgs()];');
      
      // Test 2: The transformation logic is correct
      expect(cliContent).toContain("!['generate', 'list', 'init', 'version', 'help'].includes(rawArgs[0])");
      expect(cliContent).toContain("return ['generate', ...rawArgs];");
    });
    ...
    it('should validate that Generator.generate() accepts variables parameter', () => {
      ...
      expect(generatorContent).toContain('variables?: Record<string, any>;');
  ...
  describe('Architecture Validation', () => {
    ...
    it('should have comprehensive frontmatter support as claimed', () => {
  ...
  describe('Claims Validation from HYGEN-DELTA.md', () => {
    ...
    it('should validate "Only positional parameters missing" is now resolved', () => {
      ...
      expect(cliCode).toContain('unjucks <generator> <template> [args...]');
    });

    it('should validate "Advanced Safety Features" claims', () => {
      const generatorCode = fs.readFileSync('/Users/sac/unjucks/src/lib/generator.ts', 'utf-8');
      
      expect(generatorCode).toContain('force:');
      expect(generatorCode).toContain('dry:');
      
      const fileInjectorCode = fs.readFileSync('/Users/sac/unjucks/src/lib/file-injector.ts', 'utf-8');
      expect(fileInjectorCode).toContain('backup:');
    });
