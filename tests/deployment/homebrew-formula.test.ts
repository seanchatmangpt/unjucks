...
describe('Homebrew Formula Validation', () => {
  ...
  it('should specify correct NPM registry URL', () => {
    expect(formulaContent).toContain('https://registry.npmjs.org/unjucks');
    expect(formulaContent).toMatch(/unjucks-\d+\.\d+\.\d+\.tgz/);
  });

  it('should declare Node.js as dependency', () => {
  ...
  it('should have proper metadata formatting', () => {
    // Check for proper quotes and formatting
    ...
    expect(formulaContent).toMatch(/url\s+"https?:\/\/[^"]+"/);
  });
  ...
  describe('Formula Syntax Validation', () => {
    it('should validate Ruby syntax using ruby -c if available', () => {
      try {
        ...
        const result = execSync(`ruby -c "${formulaPath}"`, { encoding: 'utf8' });
