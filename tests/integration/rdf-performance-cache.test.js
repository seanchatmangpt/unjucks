 ...
 */
describe('RDF Performance and Caching Integration Tests', () => { let dataLoader;
  let fastLoader;
  let parser;
  let rdfFilters;
  ...
  beforeEach(() => {
    ...
    const fastOptions = { ...standardOptions,
      cacheTTL };
  ...
  describe('Parsing Performance Benchmarks', () => { it('should parse small datasets within performance thresholds', async () => {
      const dataSource = {
        type };
      ...
      console.log(`Small dataset parsing - Avg)}ms, Min: ${minTime.toFixed(2)}ms, Max: ${maxTime.toFixed(2)}ms`);
    ...
    it('should parse complex schema datasets efficiently', async () => { const dataSource = {
        type };
      ...
      console.log(`Complex schema parsing)}ms for ${result.data.triples.length} triples (${triplesPerMs.toFixed(2)} triples/ms)`);
    ...
    it('should handle large datasets with acceptable performance', async () => { // Use the large-dataset.ttl fixture if available, otherwise create inline
      let dataSource;
      ...
      } catch { // Create synthetic large dataset
        const tripleCount = 5000;
        let largeTurtle = '@prefix ex } rdfs:label "Entity ${i}" ;\n`;
      ...
      console.log(`Large dataset)}ms for ${result.data.triples.length} triples`);
      console.log(`Performance)} triples/second`);
  ...
  describe('Caching Performance and Behavior', () => { it('should demonstrate significant performance improvement with caching', async () => {
      const dataSource = {
        type };
      ...
      console.log(`Cold load)}ms, Warm load: ${warmTime.toFixed(2)}ms`);
      console.log(`Speedup).toFixed(2)}x`);
    ...
    it('should handle cache expiration correctly', async () => { const shortTTLLoader = new RDFDataLoader({
        cacheEnabled,
        cacheTTL };
    ...
    it('should handle concurrent cache access safely', async () => { const dataSource = {
        type };
      ...
      console.log(`${concurrentCount} concurrent loads completed in ${totalTime.toFixed(2)}ms (avg)}ms each)`);
    ...
    it('should manage cache size and memory efficiently', async () => { const sources = [];
      
      // Create many different data sources
      for (let i = 0; i < 50; i++) {
        sources.push({
          type } ex:name "Entity ${i}" .`,
          format);
      }
      ...
      console.log(`Cache contains ${stats.size} entries, total size).toFixed(2)} KB`);
    ...
    it('should prioritize cache entries under memory pressure', async () => { // This test would require a more sophisticated cache implementation
      ...
      for (let i = 0; i < 10; i++) {
        ...
        let turtle = '@prefix ex }_${j} ex:value "${j}" .\n`;
        }
      ...
      for (const [index, source] of sources.entries()) {
        ...
        console.log(`Source ${index}, ${(end - start).toFixed(2)}ms`);
      }
      ...
      console.log(`Cached reload).toFixed(2)}ms vs original ${loadTimes[0].toFixed(2)}ms`);
    });
  });

  describe('Query Performance Optimization', () => { it('should execute queries efficiently on cached data', async () => {
      const dataSource = {
        type };
      ...
      const queryTypes = [
        ...
        () => rdfFilters.rdfQuery({ subject, predicate }
      ...
      console.log(`Query performance - Average)}ms, Max: ${maxQueryTime.toFixed(3)}ms`);
    ...
    it('should maintain query performance with large result sets', async () => { // Create dataset with many matching results
      const tripleCount = 2000;
      let largeTurtle = '@prefix ex } a foaf:Person ;\n`;
      ...
      console.log(`Large result set query).toFixed(2)}ms for ${allPersons.length} results`);
      console.log(`Filtered query).toFixed(2)}ms for ${category0.length} results`);
    ...
    it('should demonstrate filter performance with complex data structures', async () => { const complexTurtle = `
      ...
      console.log(`Results, ${dept0Employees.length} in Dept0, ${level1Employees.length} at Level1, ${intersection.length} intersection`);
  ...
  describe('Memory Efficiency and Cleanup', () => {
    it('should handle repeated cache operations without memory leaks', async () => {
      ...
      for (let cycle = 0; cycle < 5; cycle++) {
        ...
        for (let i = 0; i < 20; i++) { sources.push({
            type }_${i} ex:value "${i}" .`,
            format);
        }
        ...
        console.log(`Memory after cycle ${cycle + 1})}MB increase`);
      }
      
      const finalMemory = process.memoryUsage().heapUsed;
      const totalIncrease = (finalMemory - initialMemory) / 1024 / 1024; // MB
      
      console.log(`Total memory increase)}MB`);
      
      // Memory increase should be minimal
      expect(totalIncrease).toBeLessThan(20); // Less than 20MB increase
    });

    it('should efficiently handle cache cleanup under different load patterns', async () => { const shortTTLLoader = new RDFDataLoader({
        cacheEnabled,
        cacheTTL } ex:value ${i} .`,
          format);
      }
      ...
      console.log(`Burst load).toFixed(2)}ms`);
      ...
      for (let i = 0; i < 10; i++) { const source = {
          type } ex:value ${i} .`,
          format: 'text/turtle'
        };
