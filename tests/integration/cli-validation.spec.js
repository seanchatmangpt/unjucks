...
describe('CLI Validation Integration Tests', () => {
  ...
  constructor({{ property | camelCase }}) {
...
`
  ...
  describe('Parameter Validation', () => {
    ...
    it('should validate complex multi-parameter template', async () => { const originalCwd = process.cwd();
      process.chdir(testDir);
      ...
      try {
        ...
        expect(content).toContain('private username);
        expect(content).toContain('constructor(username)');
        expect(content).toContain('getUsername() } finally {
        process.chdir(originalCwd);
      }
    });
  ...
  describe('File Generation Validation', () => {
    ...
    it('should validate file content rendering', async () => { const originalCwd = process.cwd();
      process.chdir(testDir);

      try {
        const result = await testHelper.runCli('complex validation DataProcessor --module=processors --property=config --dataType="Record"');
        ...
        expect(content).toContain('private config, any>');
        expect(content).toContain('constructor(config)');
        expect(content).toContain('getConfig() } finally {
        process.chdir(originalCwd);
      }
    });
  });

  describe('Injection Validation', () => { it('should validate file injection with append', async () => {
      const originalCwd = process.cwd();
      process.chdir(testDir);
...
`);
        ...
        expect(content).toContain('Added } finally {
        process.chdir(originalCwd);
      }
    });
...
`);
  ...
  describe('Force Mode Validation', () => { it('should validate force overwrite behavior', async () => {
      const originalCwd = process.cwd();
      process.chdir(testDir);
      ...
      try {
        ...
        await testHelper.createFile('output/ForceTest.txt', 'Modified);
        ...
        expect(newContent).not.toContain('Modified } finally {
        process.chdir(originalCwd);
      }
    });
  });
