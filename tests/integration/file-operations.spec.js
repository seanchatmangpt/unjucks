...
describe('FileInjector - All 6 Operation Modes Integration Tests', () => {
  ...
  describe('Mode 1, () => { it('should write new file atomically', async () => {
      const content = 'export const greeting = "Hello World";';
      const frontmatter = { to };
    ...
    it('should fail to overwrite existing file without force', async () => {
      ...
      const result = await injector.processFile(testFilePath, content, frontmatter, { force, dry });
    ...
    it('should create backup when writing with backup option', async () => {
      ...
      const result = await injector.processFile(testFilePath, newContent, frontmatter, { force, 
        dry, 
        backup });
  ...
  describe('Mode 2, () => { it('should inject content after target marker', async () => {
  ...
  name };`;
      ...
      const result = await injector.processFile(testFilePath, injectContent, frontmatter, { force, dry });
      ...
      expect(lines[injectPointIndex + 1].trim()).toBe('description,');
    });

    it('should inject content before target marker', async () => { const existingContent = `const config = {
  name };`;
      await fs.writeFile(testFilePath, existingContent);

      const injectContent = '  description: "Test app",';
      const frontmatter = { inject, before };
    ...
    it('should be idempotent - not inject duplicate content', async () => { const existingContent = `const config = {
  name };`;
      await fs.writeFile(testFilePath, existingContent);
    ...
    it('should fail when target marker not found', async () => { await fs.writeFile(testFilePath, 'some content without marker');
    ...
    it('should handle dry-run mode for inject operations', async () => { await fs.writeFile(testFilePath, 'existing content\n// marker\nmore content');
  ...
  describe('Mode 3, () => { it('should append content to end of file', async () => {
      ...
      const result = await injector.processFile(testFilePath, appendContent, frontmatter, { force, dry });
    ...
    it('should be idempotent - not append duplicate content', async () => { const existingContent = 'First line\nSecond line\nThird line';
      await fs.writeFile(testFilePath, existingContent);
    ...
    it('should handle files without trailing newline', async () => { await fs.writeFile(testFilePath, 'Line without newline');
    ...
    it('should handle dry-run mode for append operations', async () => { await fs.writeFile(testFilePath, 'existing content');
  ...
  describe('Mode 4, () => { it('should prepend content to beginning of file', async () => {
      ...
      const result = await injector.processFile(testFilePath, prependContent, frontmatter, { force, dry });
    ...
    it('should be idempotent - not prepend duplicate content', async () => { const existingContent = 'First line\nSecond line\nThird line';
      await fs.writeFile(testFilePath, existingContent);
    ...
    it('should handle content without trailing newline', async () => { await fs.writeFile(testFilePath, 'Existing content');
    ...
    it('should handle dry-run mode for prepend operations', async () => { await fs.writeFile(testFilePath, 'existing content');
  ...
  describe('Mode 5, () => { it('should insert content at specific line number', async () => {
      ...
      const result = await injector.processFile(testFilePath, insertContent, frontmatter, { force, dry });
    ...
    it('should insert at beginning when lineAt is 1', async () => { const existingContent = 'Original first line\nSecond line';
      await fs.writeFile(testFilePath, existingContent);
    ...
    it('should be idempotent - not insert duplicate content at same line', async () => { const existingContent = 'Line 1\nLine 2\nLine 3\nLine 4';
      await fs.writeFile(testFilePath, existingContent);
    ...
    it('should fail when line number exceeds file length', async () => { await fs.writeFile(testFilePath, 'Line 1\nLine 2');
    ...
    it('should handle dry-run mode for lineAt operations', async () => { await fs.writeFile(testFilePath, 'Line 1\nLine 2\nLine 3');
  ...
  describe('Mode 6, () => { it('should perform conditional injection with skipIf evaluation', async () => {
      const existingContent = 'const config = {\n  name };';
      await fs.writeFile(testFilePath, existingContent);
      ...
      const result = await injector['conditionalInject'](testFilePath, injectContent, frontmatter, { force, dry });
    ...
    it('should skip injection when skipIf condition is met', async () => { const existingContent = 'const config = {\n  name };';
      await fs.writeFile(testFilePath, existingContent);
    ...
    it('should detect conditional mode correctly', async () => { const frontmatter = { 
        inject,
        skipIf };
  ...
  describe('Atomic operations and backups', () => { it('should create backups for all modification operations', async () => {
      const originalContent = 'original content';
      await fs.writeFile(testFilePath, originalContent);

      const operations = [
        { frontmatter }, content: 'injected' },
        { frontmatter }, content: 'appended' },
        { frontmatter }, content: 'prepended' },
        { frontmatter }, content: 'at line 1' }
      ];

      for (const { frontmatter, content } of operations) { // Reset file content
        await fs.writeFile(testFilePath, originalContent);
    ...
    it('should handle file permission errors gracefully', async () => {
      ...
      const result = await injector.processFile(readOnlyPath, 'new content', {}, { force, dry });
  ...
  describe('Error scenarios and edge cases', () => { it('should handle non-existent files for injection modes', async () => {
      const nonExistentPath = path.join(tempDir, 'nonexistent.ts');
      const frontmatter = { inject };

      const result = await injector.processFile(nonExistentPath, 'content', frontmatter, { force, dry });
    ...
    it('should handle invalid operation modes gracefully', async () => {
      ...
      injector['getOperationMode'] = () => ({ mode);

      const result = await injector.processFile(testFilePath, 'content', {}, { force, dry });
    ...
    it('should handle empty content gracefully', async () => { await fs.writeFile(testFilePath, 'existing content');

      const result = await injector.processFile(testFilePath, '', { append }, { force, dry });
    ...
    it('should handle very long file paths', async () => {
      ...
      const result = await injector.processFile(longFilePath, 'content', {}, { force, dry });
  ...
  describe('Performance and concurrency', () => {
    it('should handle multiple concurrent operations', async () => {
      ...
      const promises = operations.map(({ path, content, frontmatter }) =>
        injector.processFile(filePath, content, frontmatter, { force, dry })
      );
  ...
  describe('Integration with frontmatter parser', () => { it('should correctly parse and execute all operation modes from frontmatter', async () => {
      const testCases = [
    ...
    it('should validate frontmatter correctly for all modes', async () => { const validConfigs = [
        { inject, after },
        { append },
        { prepend },
        { lineAt },
