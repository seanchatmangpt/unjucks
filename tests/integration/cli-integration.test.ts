...
describe('CLI Integration Tests', () => {
  ...
  describe('unjucks list', () => {
    ...
    it('should show detailed generator information', async () => {
      await createTestGenerator('component', 'react', {
        description: 'React component generator',
        files: ['component.njk', 'test.njk', 'stories.njk']
      });
  ...
  // Helper functions
  async function createTestGenerator(generator: string, template: string, options: any = {}) {
    const generatorDir = path.join(templatesDir, generator);
...
${templateContent}`);
  ...
  async function createMalformedGenerator(generator: string, template: string) {
    const templateDir = path.join(templatesDir, generator, template);
    await fs.ensureDir(templateDir);
...
${malformedContent}`
  ...
  async function createInjectionGenerator(generator: string, template: string) {
    const templateDir = path.join(templatesDir, generator, template);
    await fs.ensureDir(templateDir);
...
---
  ...
  async function createMultiFileGenerator(generator: string, template: string) {
    const templateDir = path.join(templatesDir, generator, template);
    await fs.ensureDir(templateDir);

    // Create multiple template files
    const templates = [
      { name: 'component.njk', content: 'Component content for {{ name }}' },
      { name: 'service.njk', content: 'Service content for {{ name }}' },
      { name: 'test.njk', content: 'Test content for {{ name }}' },
      { name: 'types.njk', content: 'Types content for {{ name }}' }
    ];
    ...
    for (const template of templates) {
      await fs.writeFile(
        ...
        `---
to: src/{{ name | kebabCase }}.${template.name.split('.')[0]}
---
${template.content}`
      );
    }
