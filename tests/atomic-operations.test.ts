...
describe("Atomic File Operations", () => {
  ...
  describe("Compare-and-Swap Operations", () => {
    ...
    it("should retry compare-and-swap on transient conflicts", async () => {
      const filePath = path.join(testDir, "cas-retry.txt");
      const initialContent = "Initial content";
      ...
      (atomicOps as any).atomicRead = async (filePath: string) => {
        if (conflictCount < 2) {
          conflictCount++;
  ...
  describe("Performance and Metrics", () => {
    ...
    it("should demonstrate performance improvement", async () => {
      ...
      console.log(`Atomic operations: ${opsPerSecond.toFixed(2)} ops/sec`);
      
      // Should achieve high throughput (target: >100 ops/sec for this test)
    ...
    it("should have minimal memory overhead", async () => {
      ...
      console.log(`Memory increase: ${(memoryIncrease / 1024 / 1024).toFixed(2)}MB for ${operations} operations`);
      
      // Should have minimal memory growth (target: <10MB for 100 operations)
      expect(memoryIncrease).toBeLessThan(10 * 1024 * 1024);
    });
  });

  describe("Concurrent Safety", () => {
    it("should handle concurrent writes to different files safely", async () => {
    ...
    it("should handle concurrent operations on same file with retries", async () => {
