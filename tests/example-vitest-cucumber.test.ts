...
import { injectionStepDefinitions } from './step-definitions/injection.steps';
import { templateGenerationStepDefinitions } from './step-definitions/template-generation.steps';
import { cliCommandsStepDefinitions } from './step-definitions/cli-commands.steps';
import { commonStepDefinitions } from './step-definitions/common-steps';
import { fileOperationsStepDefinitions } from './step-definitions/file-operations.steps';
import { generatorStepDefinitions } from './step-definitions/generator-steps';
import { templateStepDefinitions } from './step-definitions/template-steps';
...
defineFeature(feature, (test) => {
  ...
  test('Basic file injection with skipIf conditions', ({ given, when, then }) => {
    given('I have a target file "src/config.ts" with content:', async (content: string) => {
      await allStepDefinitions['I have a target file {string} with content:'](world, 'src/config.ts', content);
    });

    given('I have a generator "config" with template "database.ts":', async (content: string) => {
