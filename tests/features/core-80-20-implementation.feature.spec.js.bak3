...
// {{ name | pascalCase }}Props interface
// title?: string

export const {{ name | pascalCase }} = ({ title, onClick }) => {
  return (
...
`
  ...
  test('End-to-end template generation with real file operations', ({ given, when, then }) => {
    ...
    when('I run the CLI command with real arguments', async () => { const result = await execAsync('node ../../../dist/cli.mjs generate component react --name=UserProfile', {
        cwd,
        env }
      });
      world.setLastCommandResult({ stdout });
  ...
  test('CLI commands work without mocks using positional parameters', ({ given, when, then }) => {
    ...
    when('I use Hygen-style positional syntax', async () => { const result = await execAsync('node ../../../dist/cli.mjs command citty UserManagement --required=true', {
        cwd,
        env }
      });
      world.setLastCommandResult({ stdout });
    ...
    then('the CLI list command should work without mocks', async () => { const result = await execAsync('node ../../../dist/cli.mjs list', {
        cwd,
        env }
      });
  ...
  test('Frontmatter processing with real YAML parsing', ({ given, when, then }) => {
    ...
    when('I process templates with YAML frontmatter', async () => { // Create target file for injection
      await fs.writeFile(
    ...
    then('skipIf conditions should be properly evaluated', async () => { // Test when skip is false (should generate)
      ...
      const result2 = await execAsync('node ../../../dist/cli.mjs generate test validation --template=skipif-test --skip=true', { cwd,
        env }
      });
  ...
  test('File injection operations with actual file I/O', ({ given, when, then }) => { given('I have existing files and injection templates', async () => {
    ...
    when('I perform different injection operations', async () => { // Test append injection
      ...
      await execAsync('node ../../../dist/cli.mjs generate test validation --template=prepend-inject', { cwd,
        env }
      });

      // Test line-specific injection
      await execAsync('node ../../../dist/cli.mjs generate test validation --template=line-inject', { cwd,
        env }
      });
    });
    ...
    then('injection operations should be idempotent', async () => { const contentBefore = await fs.readFile(path.join(tempDir, 'existing-file.js'), 'utf-8');
  ...
  test('File permissions and shell commands execute correctly', ({ given, when, then }) => { given('I have templates with chmod and shell commands', async () => {
      ...
      await fs.writeFile(
        ...
        `---\nto }}' > temp.txt", "cat temp.txt"]\n---\nGenerated by {{ name }}\n`
      );
    });

    when('I generate files with chmod and shell directives', async () => { const result = await execAsync('node ../../../dist/cli.mjs generate test validation --template=chmod-test --name=TestUser', {
        cwd,
        env }
      });
    ...
    then('shell commands should execute during generation', async () => { const result = await execAsync('node ../../../dist/cli.mjs generate test validation --template=shell-test --name=ShellTest', {
        cwd,
        env }
      });
  ...
  test('Critical 20% user workflows with real data', ({ given, when, then }) => { given('I have the most commonly used template patterns', async () => {
      // Component generation (most common use case)
      await fs.writeFile(
        path.join(tempDir, '_templates', 'component', 'react', 'index.ts'),
        `---\nto }}/index.ts\n---\nexport { {{ name | pascalCase }} } from './{{ name | pascalCase }}.js';\n// export type { {{ name | pascalCase }}Props } from './{{ name | pascalCase }}.js';\n`
      );
    ...
    when('I execute the most common workflow, async () => { const result = await execAsync('node ../../../dist/cli.mjs component react UserCard --withTests=true', {
        cwd,
        env }
      });
    ...
    then('the workflow should complete in under 2 seconds', () => {
      ...
      // For this test, we verify successful completion proxy for reasonable performance
    });
  });
  ...
  test('Dry run mode works correctly without file modification', ({ given, when, then }) => {
    ...
    when('I run commands with --dry flag', async () => { const result = await execAsync('node ../../../dist/cli.mjs generate component react --name=PreviewComponent --dry', {
        cwd,
        env }
      });
  ...
  test('Error handling works correctly with real scenarios', ({ given, when, then }) => {
    ...
    when('I try to generate from non-existent generator', async () => { try {
        await execAsync('node ../../../dist/cli.mjs generate nonexistent template', {
          cwd,
          env }
        });
      } catch (error) { world.setLastCommandResult({
          stdout }
    });
    ...
    when('I try to inject into a non-existent file', async () => { await fs.writeFile(
