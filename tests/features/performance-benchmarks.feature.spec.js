...
describeFeature(feature, ({ Scenario }) => {
  ...
  test('Template processing speed validation (40% faster than Hygen)', ({ given, when, then, and }) => {
    ...
    given('I have a complex template with multiple variables and conditionals:', async (templateContent) => { await performanceBenchmarksSteps['I have a complex template with multiple variables and conditionals });

    when('I benchmark template processing with variables:', async (dataTable) => { await performanceBenchmarksSteps['I benchmark template processing with variables });
  ...
  test('File operations speed validation (25% faster than Hygen)', ({ given, when, then, and }) => {
    ...
    when('I execute batch file generation:', async (command) => { await performanceBenchmarksSteps['I execute batch file generation });
  ...
  test('Large template handling performance', ({ given, when, then, and }) => {
    ...
    given('I have a template generating 10,000+ lines of code:', async (templateContent) => {
  ...
  test('Variable resolution and filter performance', ({ given, when, then, and }) => {
    ...
    given('I have a template with complex variable resolution:', async (templateContent) => { await performanceBenchmarksSteps['I have a template with complex variable resolution });
  ...
  test('Error recovery performance impact', ({ given, when, then, and }) => {
    ...
    when('I run commands that will fail:', async (dataTable) => { await performanceBenchmarksSteps['I run commands that will fail });
  ...
  test('Comparative performance validation', ({ given, when, then }) => {
    ...
    when('I run standardized benchmark suite:', async (benchmarkDescription) => { await performanceBenchmarksSteps['I run standardized benchmark suite });

    then('Unjucks should consistently outperform claimed metrics:', (dataTable) => { performanceBenchmarksSteps['Unjucks should consistently outperform claimed metrics });
