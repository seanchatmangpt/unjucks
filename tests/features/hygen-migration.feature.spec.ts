...
describeFeature(feature, ({ Scenario }) => {
  ...
  Scenario('Hygen command syntax compatibility matrix', ({ Given, When, Then, And }) => {
    let testResult: CLIResult;
    given('I set up a temporary test environment', setupWorld);

    and('I create a complete Hygen-style generator structure:', async (structureDescription: string) => {
      await hygenMigrationSteps['I create a complete Hygen-style generator structure:'](world, structureDescription);
    });

    given('I have these Hygen commands that should work in Unjucks:', async (dataTable: any) => {
      await hygenMigrationSteps['I have these Hygen commands that should work in Unjucks:'](world, dataTable);
    });
  ...
  test('EJS to Nunjucks template conversion', ({ given, when, then, and }) => {
    given('I set up a temporary test environment', setupWorld);
    ...
    given('I have a Hygen EJS template:', (ejsTemplate: string) => {
      hygenMigrationSteps['I have a Hygen EJS template:'](world, ejsTemplate);
    });

    when('I convert it to Unjucks syntax:', async (nunjucksTemplate: string) => {
      await hygenMigrationSteps['I convert it to Unjucks syntax:'](world, nunjucksTemplate);
    });
  ...
  test('Complete frontmatter compatibility with Hygen', ({ given, when, then, and }) => {
    given('I set up a temporary test environment', setupWorld);
    ...
    given('I create a template with all Hygen frontmatter features:', async (templateContent: string) => {
      await hygenMigrationSteps['I create a template with all Hygen frontmatter features:'](world, templateContent);
    });
  ...
  test('Hygen workflow patterns work in Unjucks', ({ given, when, then, and }) => {
    given('I set up a temporary test environment', setupWorld);
    ...
    given('I have a complex Hygen workflow:', (workflowDescription: string) => {
      hygenMigrationSteps['I have a complex Hygen workflow:'](world, workflowDescription);
    });

    when('I convert to Unjucks workflow:', (unjucksWorkflow: string) => {
      hygenMigrationSteps['I convert to Unjucks workflow:'](world, unjucksWorkflow);
    });
  ...
  test('Variable handling compatibility', ({ given, when, then, and }) => {
    given('I set up a temporary test environment', setupWorld);
    ...
    given('I create a template with complex variable usage:', async (templateContent: string) => {
      await hygenMigrationSteps['I create a template with complex variable usage:'](world, templateContent);
    });
  ...
  test('Migration regression testing', ({ given, when, then, and }) => {
    ...
    and('performance should improve as claimed', () => {
      hygenMigrationSteps['performance should improve as claimed'](world);
    });
