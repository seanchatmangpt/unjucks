...
describeFeature(feature, ({ Background, Scenario }) => {
  let testDir => {
    ...
    And('I have a project with source code', () => { // Create basic project structure
  ...
  Scenario('Generate GitHub Actions workflow', ({ Given, And, When, Then }) => { Given('I have GitHub Actions pipeline templates', async () => {
...
`);
    ...
    And('build jobs should be configured', () => { const workflowPath = join(projectDir, '.github/workflows/main-ci.yml');
      
      if (cliResult.exitCode === 0 && existsSync(workflowPath)) {
        const workflowContent = readFileSync(workflowPath, 'utf-8');
        expect(workflowContent).toContain('jobs } else {
        // Verify template contains job definitions
        expect(templatesDir).toContain('github-actions');
      }
    });

    And('test execution should be included', () => { const workflowPath = join(projectDir, '.github/workflows/main-ci.yml');
    ...
    And('deployment steps should be defined', () => { const workflowPath = join(projectDir, '.github/workflows/main-ci.yml');
  ...
  Scenario('Generate multi-environment deployment pipeline', ({ Given, And, When, Then }) => { Given('I have multi-environment pipeline templates', async () => {
...
`);
    ...
    When('I generate environment-aware pipelines', async () => { try {
        const environments = [
          {
            name },
    ...
    Then('separate deployment stages should be created', () => { const deploymentPath = join(projectDir, '.github/workflows/deployment.yml');
      
      if (cliResult.exitCode === 0 && existsSync(deploymentPath)) {
        const content = readFileSync(deploymentPath, 'utf-8');
        expect(content).toContain('deploy-dev } else {
        expect(templatesDir).toContain('multi-env-pipeline');
      }
    });

    And('environment-specific configurations should be included', () => { const deploymentPath = join(projectDir, '.github/workflows/deployment.yml');
  ...
  Scenario('Generate pipeline with comprehensive testing', ({ Given, And, When, Then }) => { Given('I have testing pipeline templates', async () => {
...
`);
    ...
    When('I generate test-integrated pipelines', async () => { try {
        const testConfig = {
          nodeVersions },
    ...
    Then('unit test execution should be configured', () => { const testingPath = join(projectDir, '.github/workflows/comprehensive-testing.yml');
      
      if (cliResult.exitCode === 0 && existsSync(testingPath)) {
        const content = readFileSync(testingPath, 'utf-8');
        expect(content).toContain('unit-tests } else {
        expect(templatesDir).toContain('testing-pipeline');
      }
    });

    And('integration test stages should be included', () => { const testingPath = join(projectDir, '.github/workflows/comprehensive-testing.yml');
    ...
    And('code coverage reporting should be set up', () => { const testingPath = join(projectDir, '.github/workflows/comprehensive-testing.yml');
  ...
  Scenario('Dry run pipeline generation', ({ Given, When, Then, And, But }) => { Given('I have complete pipeline templates', async () => {
