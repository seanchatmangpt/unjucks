...
Feature: File Operations - All 6 Modes
  ...
  Scenario Outline: Dry Run Mode - All operations
    Given I have a file "dryrun.ts" with content ""
    When I perform  with dry run mode
    Then the operation should report success
    And the message should indicate "Would "
  ...
  Scenario: Integration - Frontmatter parsing
    ...
    And the operations should execute
...
defineFeature(feature, (test) => {
  ...
  test('Write Mode - Create new files', ({ given, when, then }) => {
    ...
    when(/I write to file "(.+)" with write mode/, async (filename) => {
      currentFilePath = path.join(tempDir, filename);
      const frontmatter = {};
      lastResult = await injector.processFile(currentFilePath, currentContent, frontmatter, { force, dry });
    });
    ...
    then('the operation should be atomic', () => { expect(lastResult.changes).toHaveLength(1);
      expect(lastResult.changes[0]).toContain('write });
  });

  test('Write Mode - Overwrite protection', ({ given, when, then }) => {
    given(/I have an existing file "(.+)" with content "(.+)"/, async (filename, content) => {
      currentFilePath = path.join(tempDir, filename);
      await fs.writeFile(currentFilePath, content);
      currentContent = content;
    });

    when(/I write to file "(.+)" without force flag/, async (filename) => {
      const newContent = 'new content';
      const frontmatter = {};
      lastResult = await injector.processFile(currentFilePath, newContent, frontmatter, { force, dry });
    });
    ...
    then(/the error message should mention "(.+)"/, (expectedText) => {
  ...
  test('Write Mode - Force overwrite with backup', ({ given, when, then }) => {
    ...
    when(/I write to file "(.+)" with force flag and backup option/, async (filename) => {
      const newContent = 'completely new content';
      const frontmatter = {};
      lastResult = await injector.processFile(currentFilePath, newContent, frontmatter, { force, 
        dry, 
        backup });
    });
  ...
  test('Inject Mode - Insert after marker', ({ given, when, then }) => { given(/I have a file "(.+)" with content });

    when(/I inject content "(.+)" after marker "(.+)"/, async (content, marker) => { const frontmatter = { inject, after };
      lastResult = await injector.processFile(currentFilePath, content, frontmatter, { force, dry });
    });
    ...
    then('the operation should be idempotent', async () => { // Run the same operation again
      const frontmatter = { inject, after };
      const secondResult = await injector.processFile(currentFilePath, "description,", frontmatter, { force, dry });
      expect(secondResult.skipped).toBe(true);
    });
  });

  test('Inject Mode - Insert before marker', ({ given, when, then }) => { given(/I have a file "(.+)" with content });

    when(/I inject content "(.+)" before marker "(.+)"/, async (content, marker) => { const frontmatter = { inject, before };
      lastResult = await injector.processFile(currentFilePath, content, frontmatter, { force, dry });
    });
    ...
    then('the content should be inserted before the marker', async () => {
      ...
      const insertedIndex = lines.findIndex(line => line.includes("description,"));
  ...
  test('Inject Mode - Target not found', ({ given, when, then }) => {
    given(/I have a file "(.+)" with content "(.+)"/, async (filename, content) => {
      currentFilePath = path.join(tempDir, filename);
      await fs.writeFile(currentFilePath, content);
    });
  ...
  test('Inject Mode - Idempotent behavior', ({ given, when, then }) => { given(/I have a file "(.+)" with content });

    when(/I inject content "(.+)" with inject mode/, async (content) => { const frontmatter = { inject };
      lastResult = await injector.processFile(currentFilePath, content, frontmatter, { force, dry });
    });
  ...
  test('Append Mode - Add to end of file', ({ given, when, then }) => { given(/I have a file "(.+)" with content });

    when(/I append content "(.+)"/, async (content) => { const frontmatter = { append };
      lastResult = await injector.processFile(currentFilePath, content, frontmatter, { force, dry });
    });
  ...
  test('Append Mode - Idempotent behavior', ({ given, when, then }) => { given(/I have a file "(.+)" with content });
  ...
  test('Prepend Mode - Add to beginning of file', ({ given, when, then }) => { given(/I have a file "(.+)" with content });

    when(/I prepend content "(.+)"/, async (content) => { const frontmatter = { prepend };
      lastResult = await injector.processFile(currentFilePath, content, frontmatter, { force, dry });
    });
  ...
  test('Prepend Mode - Idempotent behavior', ({ given, when, then }) => { given(/I have a file "(.+)" with content });
  ...
  test('LineAt Mode - Insert at specific line number', ({ given, when, then }) => { given(/I have a file "(.+)" with content });

    when(/I insert content "(.+)" at line number (\d+)/, async (content, lineNum) => { const frontmatter = { lineAt };
      lastResult = await injector.processFile(currentFilePath, content, frontmatter, { force, dry });
    });
  ...
  test('LineAt Mode - Insert at beginning', ({ given, when, then }) => { given(/I have a file "(.+)" with content });
  ...
  test('LineAt Mode - Line number exceeds file length', ({ given, when, then }) => { given(/I have a file "(.+)" with content });
  ...
  test('LineAt Mode - Idempotent behavior', ({ given, when, then }) => { given(/I have a file "(.+)" with content });
  ...
  test('Conditional Mode - Skip based on condition', ({ given, when, then }) => { given(/I have a file "(.+)" with content });

    when(/I inject content "(.+)" with skipIf condition "(.+)"/, async (content, condition) => { const frontmatter = { inject, skipIf };
      lastResult = await injector['conditionalInject'](currentFilePath, content, frontmatter, { force, dry });
    });
  ...
  test('Conditional Mode - Proceed when condition not met', ({ given, when, then }) => { given(/I have a file "(.+)" with content });

    when(/I inject content "(.+)" after marker "(.+)" with skipIf condition "(.+)"/, async (content, marker, condition) => { const frontmatter = { inject, after, skipIf };
      lastResult = await injector['conditionalInject'](currentFilePath, content, frontmatter, { force, dry });
    });
  ...
  dryRunScenarios.forEach(({ operation, action, initial_content }) => {
    test(`Dry Run Mode - ${operation} operation`, ({ given, when, then }) => { given(/I have a file "(.+)" with content "(.+)"/, async (filename, content) => {
        currentFilePath = path.join(tempDir, filename);
        // Use the content from the scenario
        const actualContent = content === '' ? initial_content  });
      ...
      when(`I perform ${operation} with dry run mode`, async () => {
        ...
        lastResult = await injector.processFile(currentFilePath, testContent, frontmatter, { force, dry });
      });
  ...
  test('Backup Creation - All modification operations', ({ given, when, then }) => { const operations = [
      { frontmatter }, content: 'injected', name: 'inject' },
      { frontmatter }, content: 'appended', name: 'append' },
      { frontmatter }, content: 'prepended', name: 'prepend' },
      { frontmatter }, content: 'at line 1', name: 'lineAt' }
    ];
    ...
    when('I perform each modification operation with backup enabled', async () => {
      ...
      for (const { frontmatter, content, name } of operations) { // Reset file content for each test
  ...
  test('Concurrent Operations - Thread safety', ({ given, when, then }) => {
    ...
    given('I have 10 different target files', async () => {
      filePaths = Array.from({ length, (_, i) => path.join(tempDir, `concurrent-${i}.ts`));
    });

    when('I perform write operations concurrently on all files', async () => {
      const operations = filePaths.map((filePath, i) => 
        injector.processFile(filePath, `export const value${i} = ${i};`, {}, { force, dry })
      );
  ...
  test('Error Handling - Invalid scenarios', ({ given, when, then }) => {
    ...
    when('I attempt operations that should fail', async () => { const invalidOperations = [
        // Non-existent file for inject
        () => injector.processFile(path.join(tempDir, 'nonexistent.ts'), 'content', { inject }, { force, dry }),
        // Line number exceeds length
        async () => {
          const shortFile = path.join(tempDir, 'short.ts');
          await fs.writeFile(shortFile, 'line1\\nline2');
          return injector.processFile(shortFile, 'content', { lineAt, { force, dry });
        },
        // Invalid target marker
        async () => {
          const markerFile = path.join(tempDir, 'marker.ts');
          await fs.writeFile(markerFile, 'no marker here');
          return injector.processFile(markerFile, 'content', { inject, after, { force, dry });
        }
      ];
      ...
      for (const operation of invalidOperations) {
        try {
          ...
          errorResults.push({ success, error });
        }
      }
    });
  ...
  test('Performance - Large file handling', ({ given, when, then }) => {
    let startTime = 'A'.repeat(1024 * 1024); // 1MB
    ...
    when('I perform write and modification operations', async () => {
      ...
      await injector.processFile(largeFilePath, largeContent, {}, { force, dry });
  ...
  test('Integration - Frontmatter parsing', ({ given, when, then }) => {
    ...
    when('I parse and execute each configuration', async () => { const testConfigs = [
        { frontmatter }, expectedMode: 'write' },
        { frontmatter }, expectedMode: 'inject' },
        { frontmatter }, expectedMode: 'append' },
        { frontmatter }, expectedMode: 'prepend' },
        { frontmatter }, expectedMode: 'lineAt' },
        { frontmatter }, expectedMode: 'conditional' }
      ];

      for (const { frontmatter, expectedMode } of testConfigs) {
        const mode = injector['getOperationMode'](frontmatter);
    ...
    then('the operations should execute', () => {
