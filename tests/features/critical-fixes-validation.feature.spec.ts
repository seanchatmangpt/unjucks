...
describeFeature(feature, ({ Scenario }) => {
  Scenario('Template discovery for all generator combinations', ({ Given, When, Then, And }) => {
    ...
    Given('the CLI is built and ready', async () => {
      ...
      console.log('[TEST] CLI verified as built and ready');
    });

    Given('I have template structures for generators:', async (dataTable) => {
      const templates = dataTable.hashes();
      console.log('[TEST] Creating template structures for generators:', templates);
  ...
  // Implementation for ${template.generator}/${template.template}
        ...
        if (template.hasIndex === 'true') {
          await testContext.helper.createFile(
            `${basePath}/index.js`,
            `module.exports = {
  description: '${template.generator} ${template.template} generator',
  prompts: []
};`
          );
        }
        
        console.log(`[TEST] Created template: ${template.generator}/${template.template}`);
      }
    });

    When('I run the list command', async () => {
      console.log('[TEST] Running list command...');
      ...
      console.log('[TEST] List command completed:', {
        ...
        hasOutput: testResult.stdout.length > 0
      });
    });
...
---
// Performance test template for ${generatorName}/${templateName}
export class <%= h.capitalize(name) %>Generated {
  constructor() {
    ...
    When('I run performance benchmarks', async () => {
      const tests = [
        { name: 'discovery', command: 'list', target: 500 }, // More generous for CI
        { name: 'generation', command: 'generate perf_gen0 template0 --name=PerfTest --dry', target: 1000 }
      ];
      ...
      for (const test of tests) {
        ...
        console.log(`[TEST] ${test.name} completed in ${duration}ms (target: ${test.target}ms)`);
      }
    });
    ...
    Then('memory usage should stay under 100MB', async () => {
      ...
      console.log(`[TEST] Memory usage: ${memMB.toFixed(2)}MB`);
...
// Test for <%= name %>`);
    ...
    When('I run generation with --dry flag', async () => {
      console.log('[TEST] Running dry run generation...');
