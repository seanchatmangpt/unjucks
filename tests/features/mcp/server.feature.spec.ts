...
test('MCP server handles tool discovery', async () => {
  await mcpStepDefinitions['I have a clean test environment'](world);
  ...
  world.context.lastMCPResponse = {
    result: {
      tools: [
        ...
        { name: 'unjucks_help', description: 'Get help for specific generator' },
        { name: 'unjucks_generate', description: 'Generate files from templates' },
        { name: 'unjucks_dry_run', description: 'Preview generation without writing' },
        { name: 'unjucks_inject', description: 'Inject content into files' }
      ]
    }
  };
  
  const expectedTools = [
    { tool_name: 'unjucks_list', description: 'List available generators' },
    { tool_name: 'unjucks_help', description: 'Get help for specific generator' },
    { tool_name: 'unjucks_generate', description: 'Generate files from templates' },
    { tool_name: 'unjucks_dry_run', description: 'Preview generation without writing' },
    { tool_name: 'unjucks_inject', description: 'Inject content into files' }
  ];
  
  // Check that all expected tools are present
  expectedTools.forEach(expectedTool => {
    const foundTool = world.context.lastMCPResponse!.result.tools.find(
      (tool: any) => tool.name === expectedTool.tool_name
    );
    if (!foundTool) {
...
test('MCP server provides tool schemas', async () => {
  await mcpStepDefinitions['I have a clean test environment'](world);
  await mcpStepDefinitions['the MCP server is running'](world);
    ...
    result: {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          name: { type: 'string', description: 'Component/file name' },
          dest: { type: 'string', description: 'Output destination' }
        },
...
test('MCP server handles invalid requests gracefully', async () => {
  await mcpStepDefinitions['I have a clean test environment'](world);
  await mcpStepDefinitions['the MCP server is running'](world);
  ...
  world.context.lastMCPResponse = {
    error: {
      code: 400,
      message: 'Invalid tool request: missing required parameter "name"',
      data: {
        suggestions: ['Provide a name parameter', 'Check tool documentation']
      }
    }
  };
...
test('MCP server handles concurrent requests', async () => {
  ...
  const concurrentRequests = [
    ...
    world.callMCPTool('unjucks_help', { generator: 'component' }),
    world.callMCPTool('unjucks_generate', { generator: 'component', name: 'Test1' }),
    world.callMCPTool('unjucks_dry_run', { generator: 'api-route', name: 'users' })
  ];
  ...
  if (results.some(result => result.error)) {
    throw new Error('All requests should be processed successfully');
  }
...
test('MCP server validates input parameters', async () => {
  await mcpStepDefinitions['I have a clean test environment'](world);
  await mcpStepDefinitions['the MCP server is running'](world);
  
  // Test with invalid parameters
  const invalidResponse = await world.callMCPTool('unjucks_generate', {
    generator: '', // Empty generator name
    name: null,   // Null name
    dest: 123     // Wrong type for dest
  });
  
  if (!invalidResponse.error) {
    throw new Error('Server should reject the request');
  }
...
test('MCP server maintains session state', async () => {
  await mcpStepDefinitions['I have a clean test environment'](world);
  await mcpStepDefinitions['the MCP server is running'](world);
  
  // Perform multiple operations
  await world.callMCPTool('unjucks_list');
  await world.callMCPTool('unjucks_help', { generator: 'component' });
  await world.callMCPTool('unjucks_generate', { generator: 'component', name: 'TestComponent' });
  
  // Check server maintains consistency
  if (!world.context.mcpServer?.isRunning) {
    throw new Error('Server should maintain consistency');
  }
  ...
  if (tempFiles.length > 0) {
    console.warn('Temporary files may not be properly cleaned up:', tempFiles);
  }
  ...
  if (memUsageMB > 50) { // 50MB threshold
    console.warn('Resource usage may be growing:', memUsageMB, 'MB');
  }
});