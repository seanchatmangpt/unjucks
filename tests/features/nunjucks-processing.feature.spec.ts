...
describe('Nunjucks Template Processing Validation', () => {
  ...
  describe('HYGEN-DELTA Claim: Superior Nunjucks vs EJS template engine', () => {
    it('should support all Nunjucks filters claimed in HYGEN-DELTA', async () => {
...
export class {{ name | pascalCase }}Service {
  ...
  defaultValue = "{{ optionalValue | default('fallback') }}";
    ...
    it('should support template inheritance features', async () => {
...
{% endblock %}`;
      ...
      expect(content).toContain('props: any = {}');
      expect(content).toContain('render()');
      expect(content).toContain('<div>User List</div>');
    });

    it('should support macros for reusable template components', async () => {
      const templateContent = `---
to: "src/{{ name }}.ts"
---
...
export class {{ name | pascalCase }}Service {
  ...
  {{ generateMethod('delete', 'Promise<void>', ['id: string']) }}
      ...
      expect(content).toContain('create(data: Partial<User>): Promise<User>');
      expect(content).toContain('update(id: string, data: Partial<User>): Promise<User>');
      expect(content).toContain('delete(id: string): Promise<void>');
    ...
    it('should support async template processing', async () => {
      const templateContent = `---
to: "src/{{ name }}.ts"
---
export class {{ name | pascalCase }}Service {
  ...
  {% endfor %}
      ...
      expect(content).toContain('async createProduct(): Promise<Product>');
      expect(content).toContain('async readProduct(): Promise<Product>');
      expect(content).toContain('async updateProduct(): Promise<Product>');
      expect(content).toContain('async deleteProduct(): Promise<Product>');
    });

    it('should handle complex expressions and conditionals', async () => {
      const templateContent = `---
to: "src/{{ name | pascalCase }}.ts"
---
...
export class {{ name | pascalCase }}Repository {
  ...
  async find(
    ...
    {% if withFilters %}
    {{ ', filters?: Record<string, any>' if withId else 'filters?: Record<string, any>' }}
    {% endif %}
    ...
    return [];
      ...
      expect(content).not.toContain('deletedAt?: Date;');
      expect(content).toContain('private cache = new Map');
      expect(content).toContain('id: string, filters?: Record<string, any>');
      expect(content).toContain('this.cache.has(id)');
    });
  });

  describe('HYGEN-DELTA Claim: Better error handling in templates', () => {
    it('should provide detailed error messages for undefined variables', async () => {
      const templateContent = `---
to: "src/{{ name }}.ts"
---
export const {{ name }} = {
  ...
  other: "{{ anotherUndefined | someFilter }}"
    ...
    it('should validate filter usage and provide suggestions', async () => {
      const templateContent = `---
to: "src/{{ name }}.ts"
---
