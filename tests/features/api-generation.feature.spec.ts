...
to: src/controllers/{{ entityName }}Controller.js
---
...
to: src/routes/{{ entityName.toLowerCase() }}Routes.js
---
...
`);
    ...
    And('I specify an entity name "User"', () => {
      // Entity name will be passed as CLI parameter
    });
    ...
    And('I should get route definitions', () => {
      const routesPath = join(projectDir, 'src/routes/userRoutes.js');
      ...
      if (cliResult.exitCode === 0 && existsSync(routesPath)) {
        ...
        expect(routesContent).toContain("router.put('/users/:id'");
        expect(routesContent).toContain("router.delete('/users/:id'");
      } else {
...
export class {{ entityName }}Controller {
  async getAll(req, res) {
    ...
    try {
      const items = await {{ entityName }}.findAll({ userId: req.user.id });
      res.json(items);
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
...
`);
    ...
    And('I specify authentication requirements', () => {
      // Authentication config would be passed as parameters
    });
...
`;
    ...
    And('I have API route templates', async () => {
      templateBuilder = new TemplateBuilder('inject-routes', templatesDir);
