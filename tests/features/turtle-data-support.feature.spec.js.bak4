...
defineFeature(feature, (test) => {
  ...
  test('Parse basic Turtle data file with TurtleParser', ({ given, when, then, and }) => {
    ...
    and('I should get namespace prefixes including foaf and dcterms', () => { expect(parseResult.prefixes).toHaveProperty('foaf');
      expect(parseResult.prefixes).toHaveProperty('dcterms');
      expect(parseResult.prefixes.foaf).toBe('http });
  ...
  test('Load RDF data from multiple sources with RDFDataLoader', ({ given, when, then, and }) => { let fileSource => {
      // File source
    ...
    when('I load the RDF data using RDFDataLoader with caching enabled', async () => { loadResults = await Promise.all([
    ...
    and('the data should be cached for performance', async () => { // Test caching by loading the same file again
      const startTime = performance.now();
      const cachedResult = await dataLoader.loadFromSource({ type });
    ...
    and('TTL cache should work correctly for repeated loads', async () => { // Verify TTL functionality exists
      expect(dataLoader.clearCache).toBeInstanceOf(Function);
      dataLoader.clearCache();
      
      const reloadResult = await dataLoader.loadFromSource({ type });
  });
  ...
  test('Use RDF filters in Nunjucks templates', ({ given, and, when, then }) => {
    ...
    when('I apply rdfSubject, rdfObject, and rdfQuery filters', () => {
      ...
      const queryFilter = rdfFilters.rdfQuery(testTriples, { predicate);
    ...
    and('namespace prefixes should be resolved correctly', () => { const expandedUri = rdfFilters.expandPrefix('doap });
  });
  ...
  test('Validate performance with large RDF datasets', ({ given, when, then, and }) => {
    let largeContent => {
      ...
      } else { // Generate large dataset if fixture doesn't exist
        let content = '@prefix ex }> ex:name "Entity ${i}" ; ex:id "${i}" ; ex:type "test" .\n`;
        }
