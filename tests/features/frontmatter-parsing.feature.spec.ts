...
describe('Frontmatter Parsing Validation', () => {
  ...
  describe('HYGEN-DELTA Claim: Full YAML frontmatter support', () => {
    it('should parse all 10 frontmatter options correctly', async () => {
...
export class {{ name | pascalCase }}Component {
  name = "{{ name }}";
    ...
    it('should support enhanced skipIf condition syntax', async () => {
      const templateContent = `---
to: "src/{{ name }}.ts"
skipIf: "name==skip-me"
---
export const {{ name }} = "generated";`;
    ...
    it('should handle dynamic file paths with Nunjucks filters', async () => {
      const templateContent = `---
to: "src/components/{{ name | pascalCase }}Component.ts"
---
export class {{ name | pascalCase }}Component {
  constructor() {}
}`;
  ...
  describe('HYGEN-DELTA Claim: 6 injection modes vs Hygen\'s basic injection', () => {
    beforeEach(async () => {
...
// End of file`);
    ...
    it('should support inject mode with after target', async () => {
      const templateContent = `---
to: "src/existing.ts"
inject: true
after: "// Components"
...
export class {{ name | pascalCase }}Component {}`;
    ...
    it('should support inject mode with before target', async () => {
      const templateContent = `---
to: "src/existing.ts"
inject: true
before: "// Components"
...
export class {{ name | pascalCase }}Component {}`;
    ...
    it('should support append mode (Unjucks unique feature)', async () => {
      const templateContent = `---
to: "src/existing.ts"
inject: true
append: true
...
// Appended content`;
      ...
      await world.helper.createDirectory('_templates/inject/append');
      await world.helper.createFile('_templates/inject/append/content.ts', templateContent);
      
      const result = await world.helper.runCli('unjucks generate inject append');
      expect(result.exitCode).toBe(0);
      
      const content = await world.helper.readFile('src/existing.ts');
      expect(content).toEndWith('// End of file\n// Appended content');
    });

    it('should support prepend mode (Unjucks unique feature)', async () => {
      const templateContent = `---
to: "src/existing.ts"
inject: true
prepend: true
...
// Prepended content`;
      ...
      await world.helper.createDirectory('_templates/inject/prepend');
      await world.helper.createFile('_templates/inject/prepend/content.ts', templateContent);
      
      const result = await world.helper.runCli('unjucks generate inject prepend');
      expect(result.exitCode).toBe(0);
      
      const content = await world.helper.readFile('src/existing.ts');
      expect(content).toStartWith('// Prepended content\n// Imports');
    });

    it('should support lineAt mode (Unjucks unique feature)', async () => {
      const templateContent = `---
to: "src/existing.ts"
inject: true
lineAt: 3
...
// Inserted at line 3`;
      ...
      await world.helper.createDirectory('_templates/inject/lineat');
      await world.helper.createFile('_templates/inject/lineat/content.ts', templateContent);
      
      const result = await world.helper.runCli('unjucks generate inject lineat');
      expect(result.exitCode).toBe(0);
      
      const content = await world.helper.readFile('src/existing.ts');
      const lines = content.split('\n');
      expect(lines[2]).toBe('// Inserted at line 3'); // 0-indexed
    });
  });

  describe('HYGEN-DELTA Claim: Enhanced shell command support', () => {
    it('should support array of shell commands', async () => {
      const templateContent = `---
to: "src/{{ name }}.ts"
...
export const {{ name }} = "test";`;
  ...
  describe('HYGEN-DELTA Claim: chmod support (Unjucks unique)', () => {
    it('should set file permissions via chmod frontmatter', async () => {
      const templateContent = `---
to: "scripts/{{ name }}.sh"
chmod: "755"
---
