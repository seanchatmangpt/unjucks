...
// Load feature files
const features = loadFeatures('tests/features/*.feature', 'tests/features/cucumber-step-definitions.ts');
...
describe('BDD Integration Tests', () => {
  ...
  features.forEach(feature => {
    describeFeature(feature, ({ scenario, given, when, then }) => {
      ...
      scenario('Initialize AI swarm with mesh topology', ({ given, when, then }) => {
        ...
        when('I run "unjucks swarm init --topology mesh --agents 5"', async () => {
      ...
      scenario('Execute semantic validation on valid RDF file', ({ given, when, then }) => {
        ...
        when('I run "unjucks semantic validate ontology.ttl"', async () => {
      ...
      scenario('Execute semantic validation on invalid RDF file', ({ given, when, then }) => {
        ...
        when('I run "unjucks semantic validate invalid.ttl"', async () => {
          try {
            await context.mcpClient.callTool('semantic_validate', {
              file: 'invalid.ttl',
      ...
      scenario('Generate semantic templates with RDF integration', ({ given, when, then }) => {
        ...
        when('I run "unjucks generate semantic-model --schema user-schema.ttl --output ./src/models"', async () => {
          await context.mcpClient.callTool('template_generate', {
      ...
      scenario('Spawn multiple agents for parallel processing', ({ when, then }) => {
        when('I run "unjucks swarm spawn --type coder --count 3"', async () => {
      ...
      scenario('Execute task orchestration across swarm', ({ given, when, then }) => {
        given('I have an active swarm with 5 agents', async () => {
          await context.mcpClient.callTool('swarm_init', {
            topology: 'mesh',
            maxAgents: 5
          });
        ...
        when('I run "unjucks swarm execute --task \'analyze codebase\' --strategy parallel"', async () => {
          await context.mcpClient.callTool('task_orchestrate', {
  ...
  it('should store BDD setup information in memory', async () => {
    const setupInfo = {
      ...
      scenarios: features.reduce((count, feature) => count + feature.children.length, 0),
    ...
    await context.mcpClient.callTool('memory_store', {
      key: 'swarm/bdd/setup',
