...
describe('Load Testing Performance', () => { const testDir = join(process.cwd(), 'tests/temp/load-testing');
  ...
  function setupLoadTestTemplates() { // Stress test template with many variables and conditionals
    const stressDir = join(testDir, '_templates/stress/test');
    mkdirSync(stressDir, { recursive });
...
export class <%= h.changeCase.pascal(name) %>Service {
  ...
  private metrics = { requests };
  
  private updateMetrics(responseTime, isError = false) {
  ...
  public async <%= method %>Data(
    {% if method === 'get' %}id: string{% else %}data) {
    ...
    try {
      ...
      {% elif feature === 'security' %}
      {% if method !== 'get' %}
      data = Object.keys(data).reduce((acc, key) => { acc[key] = typeof data[key] === 'string' ? this.sanitize(data[key])  }, {});
      {% endif %}
      {% endif %}
      {% endfor %}
      
      // Simulate processing
      const result = { method }id{% else %}data{% endif %},
...
`);
    ...
    mkdirSync(iterationDir, { recursive });
...
export const process<%= h.changeCase.pascal(name) %> = () => { const items = Object.values(<%= h.changeCase.camel(name) %>Data);
...
`);
  ...
  it('should efficiently handle large iteration templates', async () => { const testCases = [
      { itemCount },
      { itemCount },
      { itemCount }
    ];
  ...
  it('should handle error recovery efficiently', async () => { const measurements }[] = [];
...
`);
    ...
    for (const { template, name, shouldSucceed } of testSequence) {
      ...
      if (shouldSucceed) { mkdirSync(outputDir, { recursive });
      }
      
      const startTime = performance.now();
      
      try {
        await execAsync(`node ${cliPath} generate ${template} --name ${name} --dest ${outputDir || testDir}`, { cwd,
          timeout);
        
        const endTime = performance.now();
        
        if (shouldSucceed) {
          measurements.push({ success } else {
    ...
    if (successTimes.length > 0) {
      ...
      console.log(`  Success average)}ms`);
    ...
    if (errorTimes.length > 0) {
      ...
      console.log(`  Error detection average)}ms`);
