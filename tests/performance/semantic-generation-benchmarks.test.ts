...
describe('Semantic Generation Performance Benchmarks', () => {
  ...
  describe('Template Rendering Performance', () => {
    ...
    it('should render medium datasets (1k-10k entities) within 1 second', async () => {
      const mediumDataset = generateDataset(5000);
      const template = createPerformanceTemplate();
      ...
      const entityCount = (result.match(/a org:Person/g) || []).length;
      expect(entityCount).toBe(5000);
    });

    it('should render large datasets (10k-50k entities) within 5 seconds', async () => {
      const largeDataset = generateDataset(25000);
      const template = createPerformanceTemplate();
      ...
      expect(result.split('a org:Person').length - 1).toBe(25000);
    });

    it('should handle enterprise datasets (50k+ entities) within 15 seconds', async () => {
      const enterpriseDataset = generateDataset(75000);
      const template = createPerformanceTemplate();
  ...
  describe('Memory Usage Benchmarks', () => {
    it('should maintain reasonable memory usage for large datasets', async () => {
      ...
      for (const size of sizes) {
        ...
        memoryUsages.push({
          size,
          renderMemoryDelta: afterRender.heapUsed - beforeRender.heapUsed,
          parseMemoryDelta: afterParse.heapUsed - beforeParse.heapUsed,
          totalMemory: afterParse.heapUsed,
          tripleCount: store.size
        });
  ...
  describe('Complex Template Performance', () => {
    it('should handle templates with complex logic efficiently', async () => {
      const complexData = {
        entities: generateDataset(5000).people,
        relationships: generateRelationships(5000, 15000), // 3x more relationships than entities
        computedFields: true,
        aggregations: true
      };
...
{% endif %}`;
    ...
    it('should benchmark filter operations performance', async () => {
      const filterData = {
...
{% endfor %}`;
  ...
  describe('Scalability Stress Tests', () => {
    ...
    it('should benchmark concurrent template processing', async () => {
  ...
  function generateRelationships(entityCount: number, relationshipCount: number) {
    const relationships = [];
    const relationshipTypes = ['worksOn', 'mentors', 'collaboratesWith', 'reportsTo'];
    
    for (let i = 0; i < relationshipCount; i++) {
      relationships.push({
        from: `person-${i % entityCount}`,
        to: `person-${(i + 1) % entityCount}`,
