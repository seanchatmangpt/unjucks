...
describe('Turtle Parser Performance Tests', () => {
  ...
  describe('Large Dataset Performance', () => { it('should parse 1000 entities within 2 seconds', async () => {
      // Generate large Turtle dataset
      let turtleContent = '@prefix ex }> ex:name "User ${i}" ; ex:id "${i}" ; ex:email "user${i}@example.com" .\n`;
      }
    ...
    it('should parse 5000 entities within 10 seconds', async () => { // Generate very large dataset
      let turtleContent = '@prefix ex }> `;
    ...
    it('should handle complex nested structures efficiently', async () => { // Generate complex nested data
      let turtleContent = '@prefix ex }> ex:name "Project ${i}" ; ex:lead <#dev${i}> .\n`;
  ...
  describe('Memory Usage Performance', () => { it('should not leak memory with repeated parsing', async () => {
      const initialMemory = process.memoryUsage().heapUsed;

      // Parse multiple files in sequence
      for (let i = 0; i < 50; i++) {
        let turtleContent = '@prefix ex }> ex:name "Item ${j}" ; ex:batch "${i}" .\n`;
        }
      ...
      console.log(`Memory increase after 50 parsing operations).toFixed(2)}MB`);
    });

    it('should handle large single-entity datasets efficiently', async () => { // Single entity with many properties
      let turtleContent = '@prefix ex } "Value ${i}" ; `;
      }
      ...
      const entity = result.data.megaEntity;
  ...
  describe('Variable Extraction Performance', () => { it('should extract variables from large datasets efficiently', async () => {
      // Generate complex hierarchy
      let turtleContent = '@prefix ex }> ex:name "Company ${c}" .\n`;
      ...
      console.log(`Parse time)}ms, Variable extraction: ${variableTime.toFixed(2)}ms`);
      console.log(`Extracted ${variables.length} variables`);
    });

    it('should handle deep nesting efficiently', async () => { // Create deeply nested structure
      let turtleContent = '@prefix ex }_${i}`;
  ...
  describe('Concurrent Parsing Performance', () => { it('should handle multiple concurrent parsing operations', async () => {
      ...
      for (let i = 0; i < concurrentCount; i++) {
        let turtleContent = '@prefix ex }_${j}> ex:name "Item ${i}-${j}" ; ex:batch "${i}" .\n`;
        }
  ...
  describe('Error Handling Performance', () => { it('should fail fast with invalid syntax', async () => {
      const invalidTurtle = `
        @prefix ex }ms`);
    });

    it('should handle validation of large invalid files efficiently', async () => { // Create large file with error at the end
      let invalidTurtle = '@prefix ex }> ex:name "Valid ${i}" .\n`;
      }
