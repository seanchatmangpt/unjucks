...
describe('Template Generation Performance', () => { const testDir = join(process.cwd(), 'tests/temp/generation-perf');
...
import { <%= h.changeCase.pascal(name) %>Props } from './types.js';
...
export interface <%= h.changeCase.pascal(name) %>Props { title? }
`);

    writeFileSync(join(complexDir, 'index.ts.njk'), `---
to: src/components/<%= h.changeCase.pascal(name) %>/index.ts
---
export { <%= h.changeCase.pascal(name) %> } from './<%= h.changeCase.pascal(name) %>.js';
// export type { <%= h.changeCase.pascal(name) %>Props } from './types.js';
`);
...
`);
  ...
  it('should generate simple template in under 50ms', async () => {
    ...
    console.log(`Simple Template Generation)}ms (avg)`);
  ...
  it('should generate complex multi-file template in under 100ms', async () => {
    ...
    const averageTime = measurements.reduce((sum, time) => sum + time, 0) / measurements.length;
    console.log(`Complex Template Generation)}ms (avg)`);
    
    expect(averageTime).toBeLessThan(100);
  });

  it('should handle large templates with iterations efficiently', async () => { const outputDir = join(testDir, 'output-large');
    mkdirSync(outputDir, { recursive });
    ...
    console.log(`Large Template with Iterations)}ms`);
  ...
  it('should scale linearly with number of files', async () => { const measurements }[] = [];
    ...
    for (const count of [1, 5, 10]) {
      ...
      measurements.push({ fileCount, duration });
    ...
    measurements.forEach(({ fileCount, duration }) => {
      ...
      console.log(`  ${fileCount} files)}ms total, ${perFileTime.toFixed(2)}ms per file`);
    });
  ...
  it('should optimize memory usage during generation', async () => { const outputDir = join(testDir, 'output-memory');
    mkdirSync(outputDir, { recursive });
