...
describe('BDD Test Suite Validation', () => {
  ...
  describe('Feature Files Structure', () => { it('should have all required feature files', async () => {
    ...
    it('should have valid Gherkin syntax in feature files', async () => { const featureFiles = fs.readdirSync(featuresDir).filter(f => f.endsWith('.feature'));
      ...
      for (const file of featureFiles) {
        ...
        expect(content).toMatch(/^\s*Feature }
          ...
          if (inScenario && trimmed && !trimmed.startsWith('#')) { // Should be a step or scenario-related keyword
            if (!trimmed.match(/^(Given|When|Then|And|But|Background });
            }
          }
        }
      }
...
describe('BDD Suite Execution Test', () => {
  ...
  it('should be able to import step definitions without errors', async () => { const stepFiles = [
      './step-definitions/cli-core.steps.js', // Note }).not.toThrow();
      }
    }
  });
});
