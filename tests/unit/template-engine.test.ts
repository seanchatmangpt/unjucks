...
describe('TemplateEngine', () => {
  ...
  let mockFileSystem: FileSystemHelper;
  let tempDir: string;

  beforeEach(async () => {
    tempDir = '/tmp/test-' + Date.now();
    mockFileSystem = new FileSystemHelper(tempDir);
    engine = new TemplateEngine({ templatesPath: tempDir + '/_templates' });
  });
  ...
  describe('Template Rendering', () => {
    ...
    it('should provide utility filters', async () => {
      ...
      const data = { name: 'user profile', description: 'User Profile Component' };
    ...
    it('should handle conditional rendering', async () => {
      ...
      // Act & Assert
      const withBoth = await engine.render(template, { showHeader: true, showFooter: true });
      expect(withBoth).toContain('Header Content');
      expect(withBoth).toContain('Footer Content');

      const headerOnly = await engine.render(template, { showHeader: true, showFooter: false });
...
export const <%= name %> = () => {};`;
    ...
    it('should render frontmatter with variables', async () => {
      // Arrange
      const templateWithDynamicFrontmatter = `---
to: src/<%= module %>/<%= name %>.ts
---
export class <%= name %> {}`;
      const data = { module: 'services', name: 'UserService' };
...
Name: <%= name %>, Age: <%= age || 'Unknown' %>`;
      ...
      await mockFileSystem.createDirectory('_templates/user');
      await mockFileSystem.createFile('_templates/user/profile.ejs', template);

      // Act & Assert - Should fail without required variable
      await expect(engine.generate('user', { age: 25 })).rejects.toThrow(/required.*name/i);

      // Should succeed with required variable
      const result = await engine.generate('user', { name: 'John', age: 25 });
      expect(result.success).toBe(true);
    });
  });

  describe('Injection Support', () => {
    it('should support content injection', async () => {
      // Arrange
...
console.log('Injected content');`;
      ...
      await mockFileSystem.createDirectory('_templates/inject');
      await mockFileSystem.createFile('_templates/inject/content.ejs', injectionTemplate);

      // Act
      const result = await engine.generate('inject', {}, { targetFile: 'target.ts' });
    ...
    it('should support conditional injection with skipIf', async () => {
      // Arrange
...
inject: true
append: true
skipIf: 'console.log("already here")'
---
console.log('new content');`;

      await mockFileSystem.createDirectory('_templates/conditional');
      await mockFileSystem.createFile('_templates/conditional/inject.ejs', template);

      // Act
      const result = await engine.generate('conditional', {}, { targetFile: 'target.ts' });
  ...
  describe('Performance', () => {
    it('should handle large templates efficiently', async () => {
      ...
      const result = await engine.generate('large', { index: 1 });
    ...
    it('should cache compiled templates', async () => {
      ...
      await engine.generate('cache-test', { name: 'First' });
      const duration1 = Date.now() - start1;

      const start2 = Date.now();
      await engine.generate('cache-test', { name: 'Second' });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle templates with only frontmatter', async () => {
      // Arrange
      const onlyFrontmatter = `---
to: config.json
skipIf: true
---`;
      await mockFileSystem.createDirectory('_templates/meta-only');
      await mockFileSystem.createFile('_templates/meta-only/config.ejs', onlyFrontmatter);

      // Act
      const result = await engine.generate('meta-only', {});
    ...
    it('should handle special characters in file paths', async () => {
      // Arrange
      const template = `---
to: src/special-chars/file-with-spaces and symbols!.ts
---
export const special = true;`;

      await mockFileSystem.createDirectory('_templates/special');
      await mockFileSystem.createFile('_templates/special/index.ejs', template);

      // Act
      const result = await engine.generate('special', {});
