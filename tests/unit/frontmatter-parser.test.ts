...
Content`;
      ...
      expect(result.frontmatter.inject).toBe(true);
      expect(result.frontmatter.before).toBe('import React from \'react\';');
      expect(result.frontmatter.after).toBe('// End imports');
      expect(result.frontmatter.append).toBe(true);
      expect(result.frontmatter.prepend).toBe(false);
      expect(result.frontmatter.lineAt).toBe(15);
    });
...
Content`;
      ...
      Object.entries(frontmatter).forEach(([key, value]) => {
        if (value !== undefined) {
          expect(result.frontmatter[key as keyof typeof result.frontmatter]).toBe(value);
        }
      });
    });
  });
