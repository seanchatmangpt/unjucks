...
describe('TaskOrchestrator', () => {
  ...
  describe('Task Execution Strategies', () => {
    it('should execute tasks in parallel strategy', async () => {
      const tasks: SwarmTask[] = [
        ...
        },
        {
          ...
          type: 'template_generate',
          agentType: AgentType.CODER,
          params: { templateName: 'component', name: 'Form' },
          priority: 1,
          dependencies: [],
    ...
    it('should execute tasks in sequential strategy', async () => {
      const tasks: SwarmTask[] = [
        {
          id: 'seq-1',
          type: 'file_read',
    ...
    it('should adapt strategy based on task characteristics', async () => {
      const tasks: SwarmTask[] = [
        {
          id: 'adaptive-1',
          type: 'analysis',
  ...
  describe('Load Balancing', () => {
    it('should distribute tasks across available agents', async () => {
      const tasks: SwarmTask[] = Array.from({ length: 8 }, (_, i) => ({
        id: `load-task-${i}`,
        type: 'template_generate',
        agentType: AgentType.CODER,
        params: { templateName: 'test', index: i },
        priority: 1,
        dependencies: [],
        estimatedDuration: 500
      }));
    ...
    it('should handle agent failures with load rebalancing', async () => {
      const tasks: SwarmTask[] = Array.from({ length: 6 }, (_, i) => ({
        id: `failure-task-${i}`,
        type: 'template_generate',
        agentType: AgentType.CODER,
        params: { templateName: 'test', shouldFail: i === 2 }, // Make one task fail
        priority: 1,
        dependencies: [],
        estimatedDuration: 300
      }));
  ...
  describe('Agent Management', () => {
    ...
    it('should handle agent capacity limits', async () => {
      // Create orchestrator with limited capacity
      const limitedOrchestrator = new TaskOrchestrator({
        memoryInterface,
        workspace: testWorkspace,
        maxConcurrent: 2, // Limit to 2 concurrent tasks
        timeout: 5000
      });

      await limitedOrchestrator.initialize();

      const tasks: SwarmTask[] = Array.from({ length: 5 }, (_, i) => ({
        id: `capacity-task-${i}`,
        type: 'template_generate',
        agentType: AgentType.CODER,
        params: { templateName: 'test' },
        priority: 1,
        dependencies: [],
        estimatedDuration: 1000
      }));
      ...
      const tasks: SwarmTask[] = [
        {
          ...
          agentType: AgentType.CODER,
          params: { duration: 15000 }, // 15 seconds, longer than orchestrator timeout
          priority: 1,
          dependencies: [],
      ...
      const tasks: SwarmTask[] = [
        {
          ...
          agentType: AgentType.CODER,
          params: { failureRate: 0.7 }, // 70% chance of failure
          priority: 1,
          dependencies: [],
          estimatedDuration: 200
        }
  ...
  describe('Memory Integration', () => {
    it('should share task results through memory interface', async () => {
      const tasks: SwarmTask[] = [
        {
          id: 'producer-task',
          type: 'data_generation',
          agentType: AgentType.RESEARCHER,
          params: { outputKey: 'shared-data' },
          priority: 2,
          dependencies: [],
          estimatedDuration: 400
        },
    ...
    it('should clean up temporary memory after task completion', async () => {
      const tasks: SwarmTask[] = [
        {
          id: 'temp-memory-task',
          type: 'temporary_operation',
          agentType: AgentType.CODER,
          params: { createTempData: true },
          priority: 1,
          dependencies: [],
          estimatedDuration: 200
        }
