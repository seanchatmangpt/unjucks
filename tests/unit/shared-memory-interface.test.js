...
describe('SharedMemoryInterface', () => {
  ...
  afterEach(async () => {
    ...
    if (existsSync(testPersistPath)) { rmSync(testPersistPath, { recursive: true, force });
    }
  });

  describe('Basic Operations', () => { it('should store and retrieve simple values', async () => {
    ...
    it('should handle complex nested objects', async () => { const complexValue = {
        user },
                performance: { caching }
              }
            }
  ...
  describe('Namespacing', () => {
    it('should isolate data by namespace', async () => {
      await memoryInterface.set('shared-key', 'namespace-1-value', { namespace);
      await memoryInterface.set('shared-key', 'namespace-2-value', { namespace);

      const value1 = await memoryInterface.get('shared-key', { namespace);
      const value2 = await memoryInterface.get('shared-key', { namespace);
    ...
    it('should list keys by namespace', async () => {
      await memoryInterface.set('key1', 'value1', { namespace);
      await memoryInterface.set('key2', 'value2', { namespace);
      await memoryInterface.set('key3', 'value3', { namespace);
      await memoryInterface.set('key4', 'value4'); // default namespace

      const testKeys = await memoryInterface.listKeys({ namespace);
      const otherKeys = await memoryInterface.listKeys({ namespace);
    ...
    it('should clear namespace', async () => {
      await memoryInterface.set('ns-key1', 'value1', { namespace);
      await memoryInterface.set('ns-key2', 'value2', { namespace);
      await memoryInterface.set('keep-key', 'keep-value', { namespace);

      await memoryInterface.clearNamespace('clear-test');

      expect(await memoryInterface.has('ns-key1', { namespace)).toBe(false);
      expect(await memoryInterface.has('ns-key2', { namespace)).toBe(false);
      expect(await memoryInterface.has('keep-key', { namespace)).toBe(true);
    });
  });

  describe('TTL (Time To Live)', () => {
    it('should expire keys after TTL', async () => {
      await memoryInterface.set('ttl-key', 'expires-soon', { ttl); // 1 second TTL
    ...
    it('should update TTL on key access when configured', async () => { const refreshInterface = new SharedMemoryInterface({
        persistToDisk,
        defaultTTL });
  });

  describe('Access Control and Agent Isolation', () => { it('should enforce agent-based access control', async () => {
      await memoryInterface.set('agent-private', 'secret-data', {
        agentId });

    it('should allow shared access for public data', async () => { await memoryInterface.set('public-data', 'shared-info', {
        agentId });

    it('should track data ownership', async () => { await memoryInterface.set('owned-data', 'owner-info', {
        agentId });
  });
  ...
  describe('Memory Limits and Cleanup', () => {
    ...
    it('should perform LRU eviction when memory is full', async () => { const lruInterface = new SharedMemoryInterface({
        persistToDisk,
        maxMemorySize });
  });
  ...
  describe('Persistence', () => {
    ...
    it('should restore data from disk', async () => { // Store data and close interface
  ...
  describe('Search and Query', () => { beforeEach(async () => {
      // Set up test data
      await memoryInterface.set('user }, { tags, 'admin'] });
      await memoryInterface.set('user:2', { name }, { tags);
      await memoryInterface.set('config }, { tags);
      await memoryInterface.set('temp }, { tags });

    it('should search by tags', async () => { const userResults = await memoryInterface.search({ tags);
      const adminResults = await memoryInterface.search({ tags);
      
      expect(userResults.length).toBe(2);
      expect(adminResults.length).toBe(1);
      
      expect(userResults.map(r => r.key)).toContain('user });

    it('should search with key patterns', async () => { const userKeys = await memoryInterface.search({ keyPattern);
      const configKeys = await memoryInterface.search({ keyPattern);
      
      expect(userKeys.length).toBe(2);
      expect(configKeys.length).toBe(1);
      expect(configKeys[0].key).toBe('config });

    it('should search within specific namespace', async () => { const tempResults = await memoryInterface.search({ 
        namespace });

    it('should limit search results', async () => { const limitedResults = await memoryInterface.search({ 
        tags });
  });
