      ...
      await fc.assert(
        fc.property(
          fc.record({
            ...
            append: fc.boolean(),
            prepend: fc.boolean(),
            lineAt: fc.nat({ max: 1000 }),
            chmod: fc.oneof(fc.string(), fc.nat({ max: 0o777 })),
            skipIf: fc.string({ minLength: 1, maxLength: 30 }),
            content: fc.string({ minLength: 10, maxLength: 200 })
          }),
          ({ to, inject, append, prepend, lineAt, chmod, skipIf, content }) => {
            const frontmatter: FrontmatterConfig = { to, inject, append, prepend, lineAt, chmod, skipIf };
        ...
        fc.property(
          fc.record({
            invalidYaml: fc.oneof(
              fc.constant("{{{{invalid"),
              fc.constant("key: [unclosed array"),
              fc.constant("unquoted: string with: colons"),
      ...
      await fc.assert(
        fc.property(
          ...
          (content) => {
            ...
            // Property: Content without frontmatter should be handled correctly
            expect(parsed.hasValidFrontmatter).toBe(false);
            expect(parsed.content).toBe(content);
            expect(parsed.frontmatter).toEqual({});
          }
      ...
      await fc.assert(
        fc.property(
          fc.record({
            ...
            append: fc.boolean(),
            prepend: fc.boolean(),
            lineAt: fc.oneof(fc.nat({ max: 1000 }), fc.integer({ min: -10, max: 0 })),
            before: fc.string({ minLength: 1, maxLength: 20 }),
            after: fc.string({ minLength: 1, maxLength: 20 }),
            chmod: fc.oneof(
              fc.stringMatching(/^[0-7]{3,4}$/),
              fc.stringMatching(/^[0-9a-f]{1,8}$/), // Invalid format
              fc.nat({ max: 0o777 }),
              fc.nat({ min: 0o1000, max: 0o7777 }) // Invalid range
            )
          }),
          ({ inject, append, prepend, lineAt, before, after, chmod }) => {
            const frontmatter: FrontmatterConfig = { inject, append, prepend, lineAt, before, after, chmod };
      ...
      await fc.assert(
        fc.property(
          ...
          ({ variables, skipCondition }) => {
            const frontmatter: FrontmatterConfig = { skipIf: skipCondition };
      ...
      await fc.assert(
        fc.property(
          fc.oneof(
            ...
            fc.record({ inject: fc.constant(true), before: fc.string({ minLength: 1 }) }),
            fc.record({ inject: fc.constant(true), after: fc.string({ minLength: 1 }) }),
            fc.record({ lineAt: fc.integer({ min: 1, max: 100 }) }),
            fc.record({}) // Default write mode
          ),
          (frontmatter) => {
            const operationMode = frontmatterParser.getOperationMode(frontmatter);

            // Property: Should always return a valid mode
            expect(["write", "inject", "append", "prepend", "lineAt"]).toContain(operationMode.mode);

            // Property: Mode should match frontmatter settings
            if (frontmatter.lineAt !== undefined) {
              expect(operationMode.mode).toBe("lineAt");
              expect(operationMode.lineNumber).toBe(frontmatter.lineAt);
      ...
      await fc.assert(
        fc.property(
          ...
          ({ booleanVars, numberVars, stringVars }) => {
            // Property: Boolean patterns should be inferred as boolean
            for (const varName of booleanVars) {
              const variables = [{ name: varName, type: "string" as const, required: false }];
              const inferredType = templateScanner["inferVariableType"](varName);
              expect(inferredType).toBe("boolean");
            }

            // Property: Number patterns should be inferred as number
            for (const varName of numberVars) {
              const inferredType = templateScanner["inferVariableType"](varName);
              expect(inferredType).toBe("number");
            }

            // Property: String patterns should be inferred as string
        ...
        fc.property(
          fc.array(
            fc.record({
              ...
              required: fc.boolean(),
              defaultValue: fc.oneof(fc.string(), fc.boolean(), fc.nat())
            }),
          ...
          (templateVariables) => {
            ...
            // Property: Should generate args for all variables
            expect(Object.keys(cliArgs).length).toBe(templateVariables.length);

            // Property: Each variable should have corresponding CLI arg
            for (const variable of templateVariables) {
              expect(cliArgs[variable.name]).toBeDefined();
              expect(cliArgs[variable.name].description).toBeDefined();
              expect(typeof cliArgs[variable.name].description).toBe("string");

              // Property: CLI args should handle type conversion properly
              if (variable.type === "boolean") {
                expect(cliArgs[variable.name].type).toBe("boolean");
                expect(cliArgs[variable.name].default).toBe(false);
              } else {
        ...
        fc.property(
          fc.array(
            fc.record({
              name: fc.stringMatching(/^[a-zA-Z][a-zA-Z0-9_]*$/),
              type: fc.constantFrom("string", "boolean", "number"),
              value: fc.oneof(fc.string(), fc.boolean(), fc.nat({ max: 1000 }))
            }),
            { minLength: 1, maxLength: 8 }
          ),
          (testCases) => {
            const templateVariables: TemplateVariable[] = testCases.map(tc => ({
              name: tc.name,
              type: tc.type,
              required: false
            }));
              ...
              if (cliArgs[testCase.name] !== undefined) {
                ...
                switch (testCase.type) {
                  ...
                  case "number":
                    ...
                    break;
                  case "string":
                    expect(typeof convertedVars[testCase.name]).toBe("string");
                    break;
                }
              }
            }
    ...
    it("should scan template directories recursively and consistently", async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.array(
            fc.record({
              fileName: fc.stringMatching(/^[a-zA-Z][a-zA-Z0-9_.-]*\.(njk|txt|md|js|ts)$/),
              variables: fc.array(
                fc.stringMatching(/^[a-zA-Z][a-zA-Z0-9_]*$/),
                { maxLength: 5 }
              ),
