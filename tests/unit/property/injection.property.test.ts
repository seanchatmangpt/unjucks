      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async ({ filename, content }) => {
            ...
            const options: InjectionOptions = { force: true, dry: false };
    ...
    it("should be idempotent for append operations", async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.record({
            filename: fc.stringMatching(/^[a-zA-Z][a-zA-Z0-9_.-]*$/),
            initialContent: fc.string({ minLength: 5, maxLength: 100 }),
            appendContent: fc.string({ minLength: 5, maxLength: 100 })
          }),
      ...
      await fc.assert(
        fc.asyncProperty(
          fc.record({
            filename: fc.stringMatching(/^[a-zA-Z][a-zA-Z0-9_.-]*$/),
            initialContent: fc.string({ minLength: 5, maxLength: 100 }),
            prependContent: fc.string({ minLength: 5, maxLength: 100 })
          }),
      ...
      await fc.assert(
        fc.asyncProperty(
          fc.record({
            ...
            injectContent: fc.string({ minLength: 5, maxLength: 50 }),
          ...
          async ({ filename, initialLines, injectContent, targetLineIndex }) => {
            ...
            const frontmatter: FrontmatterConfig = { inject: true, after: targetLine };
            const options: InjectionOptions = { force: true, dry: false };
      ...
      await fc.assert(
        fc.asyncProperty(
          fc.record({
            ...
            chmod: fc.constantFrom("755", "644", "600", "777", 0o755, 0o644)
          }),
          async ({ filename, content, chmod }) => {
            const filePath = path.join(tmpDir, filename);
            const frontmatter: FrontmatterConfig = { chmod };
            const options: InjectionOptions = { force: true, dry: false };
            ...
            if (process.platform !== "win32") {
              ...
              // Check lower 9 bits (user, group, other permissions)
      ...
      await fc.assert(
        fc.asyncProperty(
          fc.record({
            ...
            injectContent: fc.string({ minLength: 3, maxLength: 30 }),
          ...
          async ({ filename, lines, injectContent, lineNumber }) => {
            ...
            const frontmatter: FrontmatterConfig = { lineAt: lineNumber };
            const options: InjectionOptions = { force: true, dry: false };
            ...
            if (result.success) {
              ...
              // Property: Should have one more line than original
              expect(resultLines.length).toBe(lines.length + 1);

              // Property: Injected content should be at correct position
              expect(resultLines[lineNumber - 1]).toBe(injectContent);

              // Property: Original lines should be preserved around injection
    ...
    it("should not modify files in dry run mode", async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.record({
            filename: fc.stringMatching(/^[a-zA-Z][a-zA-Z0-9_.-]*$/),
            initialContent: fc.string({ minLength: 10, maxLength: 100 }),
          ...
          async ({ filename, initialContent, newContent, operation }) => {
            ...
            const dryRunOptions: InjectionOptions = { force: true, dry: true };
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async ({ filename, initialContent, newContent }) => {
            ...
            const frontmatter: FrontmatterConfig = {};
            const options: InjectionOptions = { force: true, dry: false, backup: true };
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async ({ filename, content, invalidLineAt }) => {
            ...
            const frontmatter: FrontmatterConfig = { lineAt: invalidLineAt };
            const options: InjectionOptions = { force: true, dry: false };
