...
export const Component = () => {
  return <div>Hello</div>;
      ...
      expect(result.reason).toContain('skip condition');
...
export const Component = () => {
  return <div>Hello</div>;
    ...
    it('should handle injection with template variables', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.modifiedContent).toContain('{{ name }}: {{ name }}Component,');
    });
  ...
  describe('edge cases', () => {
    it('should handle empty files', async () => {
      ...
      const result = await injector.inject('/test/file.ts', newContent, { append: true });
    ...
    it('should handle very large files', async () => {
      ...
      const result = await injector.inject('/test/large.ts', newContent, { append: true });
    ...
    it('should handle Unicode content', async () => {
      const existingContent = `const message = "Hello 世界! 🌍";`;
    ...
    it('should handle files with mixed line endings', async () => {
      ...
      const result = await injector.inject('/test/file.ts', newContent, { lineAt: 2 });
  ...
  describe('performance', () => {
    it('should handle concurrent injections efficiently', async () => {
      ...
      mockFs.readFile.mockImplementation((filePath) => {
        const index = parseInt((filePath as string).match(/\d+/)?.[0] || '0');
        return Promise.resolve(fileContents[index]);
      });
      mockFs.writeFile.mockResolvedValue();

      const injections = fileContents.map((_, i) => 
        injector.inject(`/test/file${i}.ts`, `const new${i} = true;`, { append: true })
      );
    ...
    it('should optimize repeated marker searches', async () => {
      ...
      const startTime = Date.now();
      const result = await injector.inject('/test/file.ts', 'inserted', { after: marker });
      const endTime = Date.now();
