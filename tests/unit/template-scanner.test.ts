...
describe('TemplateScanner', () => {
  ...
  describe('variable requirements', () => {
    it('should mark conditionally used variables as optional', async () => {
      const content = `
        {{ name }}
        {% if hasDescription %}
          {{ description }}
    ...
    it('should mark loop variables as required', async () => {
      const content = `
        {% for item in items %}
          {{ item }}
        {% endfor %}
  ...
  describe('error scenarios', () => {
    ...
    it('should handle deep directory nesting', async () => {
      // Mock deep nesting: root -> sub1 -> sub2 -> sub3 -> file.njk
      mockFs.pathExists.mockResolvedValue(true);
      mockFs.readdir
        .mockResolvedValueOnce(['sub1'])
        .mockResolvedValueOnce(['sub2'])
        .mockResolvedValueOnce(['sub3'])
