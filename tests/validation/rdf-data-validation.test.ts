...
import type {
  ...
  ValidationResult,
  TurtleData as UnifiedTurtleData,
          ...
          rdfSources: [
            ...
            { rdf: { type: "uri", source: "http://example.org/data.ttl" } },
            {
              rdf: {
                type: "inline",
                source:
                  '@prefix test: <http://test.org/> . test:item test:value "test" .',
              },
            },
          ],
        };
    ...
    describe("Query Performance", () => {
      ...
      it("should handle complex queries with filters", async () => {
        ...
        const queryResult = await loader.executeQuery(result.data, {
          filter: (resource) => {
            const ageProps =
              resource?.properties?.["http://xmlns.com/foaf/0.1/age"];
            return ageProps && parseInt(ageProps?.[0]?.value ?? '0') > 25;
          },
          limit: 5,
        });
