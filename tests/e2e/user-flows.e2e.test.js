...
test.describe('User Flows E2E', () => {
  ...
  test.afterEach(async () => { // Cleanup test directory
    await fs.rm(testProjectDir, { recursive: true, force });
  });

  test.describe('First Time User Experience', () => {
    test('should guide new user through complete setup and first generation', async ({ page }) => { // Navigate to project documentation or web interface
      await page.goto('http });

    test('should provide helpful error messages for common mistakes', async ({ page }) => { // Test CLI error handling through web interface or terminal simulation
  ...
  test.describe('Component Generation Flow', () => {
    test('should complete full component generation workflow', async ({ page }) => { // Start with listing available generators
      await page.goto(`file }/index.html`); // Mock file interface
    ...
    test('should handle component generation with validation', async ({ page }) => { await page.goto(`file }/generator.html`);
  ...
  test.describe('Project Scaffolding Flow', () => {
    test('should scaffold complete project structure', async ({ page }) => { await page.goto(`file }/scaffold.html`);
    ...
    test('should handle project scaffolding with different options', async ({ page }) => { await page.goto(`file }/scaffold.html`);
  ...
  test.describe('File Injection Flow', () => {
    test('should inject code into existing files safely', async ({ page }) => { // Create existing file to inject into
      const routesFile = path.join(testProjectDir, 'src', 'routes.ts');
      await fs.mkdir(path.dirname(routesFile), { recursive });
      await fs.writeFile(routesFile, `export const routes = { home }/inject.html`);
      ...
      await page.fill('input[name="marker"]', "about);
      
      // Enter new content
      await page.fill('textarea[name="content"]', "  contact);
      ...
      await expect(preview).toContainText("about);
      await expect(preview).toContainText("contact);
      ...
      expect(modifiedContent).toContain("contact);
    });

    test('should prevent duplicate injections', async ({ page }) => { const targetFile = path.join(testProjectDir, 'src', 'index.ts');
...
import { useState } from 'react';`);
  ...
  test.describe('Error Recovery Flow', () => {
    test('should gracefully handle and recover from errors', async ({ page }) => { await page.goto(`file }/generator.html`);
    ...
    test('should handle network/filesystem errors gracefully', async ({ page }) => { // Simulate permission error by trying to write to restricted location
      await page.goto(`file }/generator.html`);
  ...
  test.describe('Multi-step Workflows', () => {
    test('should complete complex feature development workflow', async ({ page }) => { await page.goto(`file }/workflow.html`);
  ...
  // Helper functions
  async function createSampleGenerators(templatesDir) { // Create component generator
    const componentDir = path.join(templatesDir, 'component', 'react');
    await fs.mkdir(componentDir, { recursive });
...
${TemplateFactory.generateTemplateContent('test')}`);
    ...
    await fs.mkdir(serviceDir, { recursive });
...
${TemplateFactory.generateTemplateContent('service')}`);
    ...
    await fs.mkdir(pageDir, { recursive });
...
export default {{ name | pascalCase }}Page;`);
    ...
    await fs.mkdir(brokenDir, { recursive });
...
{{ unclosed_tag`);
  ...
  async function simulateCLI(command, cwd?) {
    ...
    if (command === 'unjucks generate') { return 'Error }
    return '';
  }
});