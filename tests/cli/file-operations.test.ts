...
`
    ...
    await fs.writeFile(
      '_templates/file/multi.test.ts.njk',
      `---
to: src/{{name}}.test.ts
---
...
`
    ...
    await fs.writeFile(
      ...
      `---
to: /invalid/absolute/path/{{name}}.txt
---
...
`
  ...
  describe('Basic File Creation', () => {
    it('should create a single file', async () => {
      ...
      expect(fileContent).toContain('File: TestFile');
      expect(fileContent).toContain('Content: Hello World');
      expect(fileContent).toContain('Created at: 2024-01-01');
    });
    ...
    it('should create nested directory structures', async () => {
      ...
      expect(content).toContain('Category: auth');
    });
  ...
  describe('File Overwriting and Force Mode', () => {
    it('should prompt when file exists (non-interactive test)', async () => {
      // Create initial file
    ...
    it('should preserve existing files without --force', async () => {
      // Create initial file
      await fs.mkdir('output', { recursive: true });
...
// End of file
    ...
    it('should append content to existing file', async () => {
      ...
      expect(content).toContain('// Added by injection: NewConstant');
    ...
    it('should prepend content to existing file', async () => {
      ...
      const content = await readFileContent('src/target.ts');
      expect(content).toStartWith('// Prepended: PrependedConstant');
      expect(content).toContain('export const existing');
    });
    ...
    it('should inject after specific marker', async () => {
      ...
      expect(content).toContain('// Added after marker: AfterMarker');
      expect(content).toContain('AfterMarker: string,');
    });
  ...
  describe('Error Handling in File Operations', () => {
    ...
    it('should clean up partial files on error', async () => {
      // This would test that incomplete files are cleaned up on error
  ...
  describe('File Metadata and Attributes', () => {
    ...
    it('should handle binary file generation', async () => {
      // This tests handling of binary content (though templates are usually text)
      await fs.writeFile(
