...
variables:
  ...
  - name: withTests
    type: boolean
    default, { ReactNode }{{/if}} from 'react';
...
export function {{name}}({{#if withProps}}{ children, className }) {
  return (
    {{#if (eq styling "styled-components")}}
...
variables:
  ...
  - name: skipTests
    type: boolean
    default, it, expect{{#if (eq type "components")}}, render, screen{{/if}} } from '{{#if (eq type "components")}}@testing-library/react{{else}}vitest{{/if}}';
{{#if (eq type "components")}}
import { {{name}} } from './{{name}}.jsx.js';
{{else if (eq type "api")}}
...
variables:
  ...
  - name: includeTests
    type: boolean
    default, Create{{name}}Data } from './types.js';
{{/if}}
{{#if includeComponent}}
export { {{name}} } from './{{name}}.jsx.js';
// export type { {{name}}Props } from './{{name}}.jsx.js';
{{/if}}
{{#if includeTests}}
...
`
  ...
  describe('Error Recovery in Complex Scenarios', () => {
    ...
    it('should clean up properly after interrupted operations', async () => { // This is hard to test directly, but we can test error states
      const result = await runCLI([
