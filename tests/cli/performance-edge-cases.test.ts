...
async function runCLI(args: string[] = [], cwd?: string, timeout = 30000): Promise<CLIResult> {
...
${Array.from({length: 100}, (_, i) => `{{var${i}}}: {{var${i}}}`).join('\n')}
`
    );
...
`
  ...
  describe('Resource Cleanup and Recovery', () => {
    it('should clean up temporary resources on interruption', async () => {
    ...
    it('should handle file system permission changes during execution', async () => {
      // Create output directory with normal permissions
