...
`
  ...
  describe('Flag Variations and Aliases', () => {
    ...
    it('should handle repeated flags as arrays', async () => {
  ...
  describe('Argument Precedence Rules', () => {
    ...
    it('should handle environment variable substitution', async () => {
      ...
      // Should handle env var or treat as literal
      expect(result.exitCode).toBe(0);
    });
...
module.exports = {
  variables: {
    ...
    withProps: true
      ...
      const result = await runCLI(['complex', 'variables', 'MyComponent', '--complexity', '8', '--dry']);
  ...
  describe('Performance with Complex Arguments', () => {
    ...
    it('should handle deeply nested object arguments', async () => {
      ...
      const result = await runCLI(['complex', 'nested', 'deep', '--config', deepObject, '--dry']);
