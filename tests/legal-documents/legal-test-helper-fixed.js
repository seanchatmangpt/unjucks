...
export class LegalTestHelper {
  ...
  async generateLegalBrief(data = null, outputFileName = 'test-legal-brief.tex') {
    ...
    try {
      ...
      const renderResult = await this.templateEngine.renderTemplate(templatePath, briefData);
      ...
      await fs.writeFile(outputPath, renderResult.content, 'utf-8');
  ...
  async testCertificateOfService() {
    ...
    try {
      const renderResult = await this.templateEngine.renderTemplate(templatePath, serviceData);
      
      if (!renderResult.success) {
        throw new Error(renderResult.error?.message || 'Certificate template rendering failed');
      }
