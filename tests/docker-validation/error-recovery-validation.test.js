      ...
      try {
        ...
        try {
          ...
          if (this.isolationLevel === 'item') {
            // Item-level isolation: recover but count as failure
            failedCount++;
            console.warn(`Item ${index} failed: ${processingError.message}`);
            results[index] = { error: processingError.message, recovered: true };
          } else {
...
describe('Error Recovery Validation', () => {
  ...
  describe('Circuit Breaker Pattern', () => {
    ...
    it('should transition to half-open and recover', async () => {
      ...
      // Circuit should be half-open, make 3 successful requests to close it
      for (let i = 0; i < 3; i++) {
        const result = await circuitBreaker.execute(() => mockEngine.render('test', {}));
        expect(result).toContain('Rendered: test');
      }

      // After 3 successful requests, circuit should be closed
  ...
  describe('Error Isolation in Batch Processing', () => {
    it('should isolate item-level failures', async () => {
      ...
      expect(result.statistics.processed).toBe(4);
  ...
  describe('Integration Tests', () => {
    it('should handle complex failure scenarios with multiple recovery mechanisms', async () => {
      // Create a fallback compiler that fails predictably for different templates
      const testFallbackCompiler = new FallbackCompiler();
      const originalExecute = testFallbackCompiler.executeStrategy;
      
      testFallbackCompiler.executeStrategy = async (strategy, template, context, options) => {
        if (template === 'failing-template' && strategy.name === 'primary') {
          throw new Error('Primary strategy failed for failing template');
        }
        if (template === 'critical-template') {
          throw new Error('All strategies failed for critical template');
        }
        return originalExecute.call(testFallbackCompiler, strategy, template, context, options);
      };
      ...
      const integratedProcessor = async (template, context) => {
        return await circuitBreaker.execute(async () => {
          return await retryManager.execute(async () => {
            try {
              return await testFallbackCompiler.compile(template, context);
            } catch (error) {
      ...
      expect(new Set(strategies).size).toBeGreaterThan(1); // Multiple strategies used
    });
