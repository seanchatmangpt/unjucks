 ...
 * Handles Order resource operations with enterprise-grade features
 * 
...
class OrdersController {
  ...
  setupValidationSchemas() {
    this.schemas = {
      ...
      create: Joi.object({
        // Add specific fields for Order creation
  ...
  /**
   * Get all Order with pagination and filtering
   * 
   * @swagger
   * /api//Order:
   *   get:
   *     summary: List Order
   *     description: Get paginated list of Order with optional filtering
   *     tags: [Order]
   *     security:
   *       - bearerAuth: []
   *     parameters:
  ...
  async list(req, res, next) {
    try {
      ...
      res.json(ApiResponse.success(result, 'Order retrieved successfully'));
    } catch (error) {
      next(error);
    }
  }
  ...
  async create(req, res, next) {
    try {
      ...
      logger.info(`Order created`, { id: result.id , userId: req.user?.id });

      res.status(201).json(ApiResponse.success(result, 'Order created successfully'));
    } catch (error) {
      next(error);
    }
  }
  ...
  async update(req, res, next) {
    try {
      ...
      logger.info(`Order updated`, { id: idValue.id , userId: req.user?.id });

      res.json(ApiResponse.success(result, 'Order updated successfully'));
    } catch (error) {
      next(error);
    }
  }
  ...
  async delete(req, res, next) {
    try {
      ...
      logger.info(`Order deleted`, { id: value.id , userId: req.user?.id });
