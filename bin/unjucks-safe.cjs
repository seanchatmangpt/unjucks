...
/**
 * Unjucks CLI Entry Point - Safe Version
...
const fs = require('fs');
const cliPath = path.join(__dirname, '../src/cli/index-safe.js');
...
async function loadAndRunCLI() {
  try {
    ...
    const fullPath = path.resolve(__dirname, '../src/cli/index-safe.js');
