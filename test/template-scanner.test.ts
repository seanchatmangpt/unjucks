...
describe("TemplateScanner", () => {
  ...
  describe("scanTemplate", () => {
    it("should scan simple variables in template files", async () => {
      ...
      await helper.createFile("template/Component.tsx", `
...
`);
      ...
      const result = await scanner.scanTemplate(path.join(helper.getTempDir(), "template"));

      // Then: Should find the variables
      expect(result.variables).toHaveLength(2);
      expect(result.variables.find(v => v.name === "componentName")).toBeDefined();
...
`);
    ...
    it("should scan variables in filenames", async () => {
      ...
      await helper.createFile("template/{{ componentName }}.tsx", `
import React from 'react';
...
`);
      ...
      expect(result.variables).toHaveLength(2);
      expect(result.variables.find(v => v.name === "componentName")).toBeDefined();
      expect(result.variables.find(v => v.name === "withTests")).toBeDefined();
    });
...
`);
  ...
  describe("generateCliArgs", () => {
    it("should generate CLI arguments from template variables", () => {
      ...
      const variables = [
        { name: "componentName", type: "string" as const, description: "Component Name", required: true },
        { name: "withTests", type: "boolean" as const, description: "Include tests", required: false },
        { name: "itemCount", type: "number" as const, description: "Number of items", required: false },
      ];
