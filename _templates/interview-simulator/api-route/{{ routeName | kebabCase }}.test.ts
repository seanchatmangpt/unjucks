...
vi.mock('./{{ routeName | kebabCase }}.{{ method | lower }}', () => ({
  default: mockHandler
}))
...
describe('{{ routeName | pascalCase }} {{ method }} API Route', () => {
  let mockEvent: any
  {% if withAuth %}let mockUser: any{% endif %}
  {% if withDatabase %}let mockDb: any{% endif %}
  ...
  describe('GET {{ routeName | kebabCase }}', () => {
    it('returns {{ routeName }} list successfully', async () => {
      const mockData = [
        {
          id: 1,
    ...
    it('requires authentication', async () => {
      mockVerifyAuth.mockResolvedValue({ user: null })
  ...
  {% elif method == 'POST' %}
  describe('POST {{ routeName | kebabCase }}', () => {
    const validBody = {
      title: 'New Interview',
    ...
    it('creates {{ routeName | singularize }} successfully', async () => {
      ...
      const createdRecord = {
        id: 1,
        ...validBody,
        {% if withAuth %}userId: mockUser.id,{% endif %}
        createdAt: new Date(),
        updatedAt: new Date()
      }
    ...
    it('generates AI interview questions when not provided', async () => {
      const bodyWithoutQuestions = {
        ...validBody,
        type: 'technical',
        difficulty: 'medium'
      }
  ...
  describe('{{ method }} {{ routeName | kebabCase }}', () => {
    const updateBody = {
  ...
  describe('DELETE {{ routeName | kebabCase }}', () => {
    it('deletes {{ routeName | singularize }} successfully', async () => {
      ...
      {% if withDatabase %}
      mockDb.{{ routeName | snakeCase | singularize }}.delete.mockResolvedValue({ id: 1 })
      {% endif %}
  ...
  describe('Error Handling', () => {
    ...
    it('preserves authentication errors', async () => {
      ...
      mockVerifyAuth.mockRejectedValue(authError)
  ...
  describe('AI Integration', () => {
    it('generates appropriate questions based on type and difficulty', async () => {
      const body = {
