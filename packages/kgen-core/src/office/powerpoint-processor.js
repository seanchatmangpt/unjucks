/**
 * Complete PowerPoint processor for KGEN using pptxgenjs library
...
class PowerPointProcessor {
  ...
  handleError(type, error, context = {}) {
    ...
    this.errors.push({ type, error, context, timestamp: this.getDeterministicTimestamp() });
    return { success: false, error: error.message, context };
  }
  ...
  configurePresentationProperties(pptx, config) {
    ...
    pptx.author = config.author || 'KGEN PowerPoint Processor';
  ...
  async applyFinalConfigurations(presentation, options) {
    ...
    pptx.revision = String(this.getDeterministicTimestamp());
    pptx.company = presentation.config.company || 'Generated by KGEN';
  }
