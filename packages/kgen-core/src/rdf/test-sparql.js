/**
 * SPARQL Engine Test Suite
 * 
 * Tests the ported SPARQL engine with actual RDF data to validate:
 * - Query execution returns real results (not empty)
 * - Performance meets <150ms target
 * - Multiple output formats work correctly
 * - Semantic hashing functions properly
 */
...
async function runTests() {
  consola.info('Starting SPARQL Engine Test Suite...');
  ...
  try {
    ...
    const semanticHasher = new SemanticHasher({
      performanceTarget: 10, // 10ms target for tests
    ...
    await runTest('Basic SELECT Query', async () => {
      ...
      consola.info(`Results: ${JSON.stringify(results.results?.bindings?.slice(0, 3), null, 2)}`);
    ...
    await runTest('CONSTRUCT Query', async () => {
      const query = `
        PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
      ...
      consola.info(`CONSTRUCT query executed in ${executionTime.toFixed(2)}ms`);
      consola.info(`Constructed ${results.quads?.length || 0} triples`);
      
      if (!results.quads || results.quads.length === 0) {
        consola.warn('CONSTRUCT query returned no triples - may be expected if no rdf:type triples exist');
      }
      
      return `Constructed ${results.quads?.length || 0} triples in ${executionTime.toFixed(2)}ms`;
    }, testResults);
    ...
    await runTest('Output Format Support', async () => {
      ...
      if (!jsonResults.results?.bindings) {
    ...
    // Test 5: Semantic hashing performance
    await runTest('Semantic Hashing Performance', async () => {
      const parseResult = await sparqlEngine.canonicalProcessor?.parseRDF?.(testRdfData, 'turtle') || 
                          { quads: sparqlEngine.store.getQuads() };
      
      if (!parseResult.quads || parseResult.quads.length === 0) {
        throw new Error('No RDF quads available for hashing test');
      }
      
      const startTime = performance.now();
      const hashResult = await semanticHasher.generateHash(parseResult.quads);
      const executionTime = performance.now() - startTime;
      ...
      if (executionTime > 50) {
        throw new Error(`Hashing took ${executionTime.toFixed(2)}ms (target: <50ms)`);
      }
      
      return `Generated hash in ${executionTime.toFixed(2)}ms for ${parseResult.quads.length} triples`;
    }, testResults);
    
    // Test 6: Query cache functionality
    await runTest('Query Caching', async () => {
      ...
      // Cache should be faster (though with small queries, difference may be minimal)
      return `Cache hit achieved. First: ${time1.toFixed(2)}ms, Second: ${time2.toFixed(2)}ms`;
    }, testResults);
    
    // Test 7: Engine status and metrics
    await runTest('Engine Status', async () => {
      ...
      return `Engine operational with ${status.metrics.totalQueries} total queries`;
    }, testResults);
