...
interface TestContext {
  gitRepoPath: string;
  provenanceTracker: any;
  gitIntegration: GitProvenanceIntegration;
  attestationGenerator: AttestationGenerator;
  cryptoVerifier: CryptographicVerifier;
  testArtifacts: Map<string, string>;
  testAttestations: Map<string, any>;
  signingKeys: Map<string, any>;
  lastGeneratedAttestation: any;
...
Given('a git repository with KGEN configured at {string}', async function(repoPath: string) {
...
Given('I have a template file {string} in git', async function(templateFile: string) {
...
Given('I generate an artifact {string} from the template', async function(artifactFile: string) {
...
Then('an {string} file should be created alongside {string}', async function(attestationSuffix: string, artifactFile: string) {
...
Given('an artifact {string} with SHA256 hash {string}', async function(artifactFile: string, expectedHash: string) {
  const artifactPath = path.join(testContext.gitRepoPath, artifactFile);
...
Then('the RSA signature should be {int} bytes in length', async function(expectedLength: number) {
  const attestation = testContext.lastGeneratedAttestation;
  const jwsParts = attestation.signature.split('.');
...
Given('I generate artifact {string} from {string}', async function(artifactFile: string, templateFile: string) {
  const artifactPath = path.join(testContext.gitRepoPath, artifactFile);
  const generatedContent = `
import React from 'react';
...
Then('it should include template dependency chain:', async function(dataTable) {
  ...
  for (const expectedTemplate of expectedTemplates) {
    const templateEntry = attestation.template_hierarchy.find(
      (t: any) => t.template === expectedTemplate.template
    );
...
Given('an artifact {string} with attestation', async function(artifactFile: string) {
  const artifactPath = path.join(testContext.gitRepoPath, artifactFile);
...
Then('the note namespace should be {string}', async function(expectedNamespace: string) {
...
Then('the error should specify {string}', async function(expectedErrorCode: string) {
...
Given('a git repository with {int}+ files and attestations', async function(fileCount: number) {
...
Then('the query should complete in under {int} seconds', async function(maxSeconds: number) {
  expect(testContext.lastVerificationResult.queryTime).to.be.below(maxSeconds * 1000);
});

Then('memory usage should remain below {int}MB', async function(maxMemoryMB: number) {
