...
const testConfig: TestConfig = {
  ...
  logLevel: process.env.TEST_LOG_LEVEL as any || 'info',
...
// Extend Cucumber World with common utilities
declare module '@cucumber/cucumber' {
  interface World {
    scenarioStartTime?: number;
    scenarioStartMemory?: number;
    scenarioName?: string;
    stepCount?: number;
...
World.prototype.logDebug = function(message: string) {
  if (testConfig.logLevel === 'debug') {
    console.log(`[DEBUG] ${message}`);
  }
};

World.prototype.logInfo = function(message: string) {
  if (['debug', 'info'].includes(testConfig.logLevel)) {
    console.log(`[INFO] ${message}`);
  }
};

World.prototype.logWarn = function(message: string) {
  if (['debug', 'info', 'warn'].includes(testConfig.logLevel)) {
    console.warn(`[WARN] ${message}`);
  }
};

World.prototype.logError = function(message: string) {
  console.error(`[ERROR] ${message}`);
};

World.prototype.expectWithinTimeout = async function<T>(
  fn: () => Promise<T> | T,
  timeout: number,
  errorMessage?: string
): Promise<T> {
